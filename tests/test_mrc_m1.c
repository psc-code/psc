...
{
  struct mrc_patch *patches = mrc_domain_get_patches(m1->domain, NULL);
  struct mrc_crds *crds = mrc_domain_get_crds(m1->domain);
...
test_write_read_m1(struct mrc_m1 *m1)
{
  ...
  struct mrc_m1 *m1_2 = mrc_m1_read(io, mrc_m1_name(m1));
  mrc_io_close(io);
  mrc_io_destroy(io);
...
main(int argc, char **argv)
{
  ...
  mrc_m1_set_param_int(m1, "nr_comps", 2);
