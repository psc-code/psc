...
ggcm_mhd_crds_c_setup(struct ggcm_mhd_crds *crds)
{
  ...
  for (int d = 0; d < 3; d++) {
    int nr_comps = mrc_f1_nr_comps(crds->f1[d]);
    mrc_f1_set_param_int_array(crds->f1[d], "offs", 2, (int [2]) { -BND, 0 });
    mrc_f1_set_param_int_array(crds->f1[d], "dims", 2, (int [2]) { info.ldims[d] + 2*BND, nr_comps });
  }
