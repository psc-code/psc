...
ggcm_mhd_ic_ici_run(struct ggcm_mhd_ic *ic)
{
  ...
  mrc_f3_foreach(f3, ix, iy, iz, 1, 1) {
   ...
   // based on Sullivan, Bhattacharjee & Huang 2009
    ...
    MRC_F3(f3, _B1X , ix, iy, iz) = cos(ky*r[1])*sin(kx*r[0]);
    MRC_F3(f3, _B1Y , ix, iy, iz) = -cos(kx*r[0])*sin(ky*r[1]); 
    MRC_F3(f3, _RR1, ix, iy, iz) = sub->n0 +   
      0.5 * (1.0 - sqrt(sqr( MRC_F3(f3, _B1X , ix, iy, iz))
			+ sqr( MRC_F3(f3, _B1X , ix, iy, iz))));
    MRC_F3(f3, _RV1X , ix, iy, iz) = sub->v0*sin(ky*r[1]) * MRC_F3(f3, _RR1, ix, iy, iz);
    MRC_F3(f3, _RV1Y , ix, iy, iz) = sub->v0*sin(kx*r[0]) * MRC_F3(f3, _RR1, ix, iy, iz);
    MRC_F3(f3, _UU1 , ix, iy, iz) = MRC_F3(f3, _RR1, ix, iy, iz)/ 
      (gamma - 1.f) +
