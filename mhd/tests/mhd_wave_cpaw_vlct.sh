...
#PBS -j oe
#
# Arguments: ./mhd_wave_cpaw_vlct.sh [pdim [mode_num [ncells [v_par]]]]
...
#
# Run Notes: Circularly polarized alfven wave. This test does not look good
#            in 1D or 2D
#
...
run_name="cpaw_vlct_${pdim}_${ncells}"

if [ $pdim = 'x' ]; then 
  hx=1.0;          hy=0.1;          hz=0.1
  dmx=${ncells};   dmy=4;           dmz=1  # domain m[xyz]
...
# viscid_ts --slice x=0,y=0,z=0 -p vy -p vz -p by -p bz cpaw_vlct_x_512.3d.xdmf

# plot final time step:
# viscid_2d --own --slice z=0 -p vy -p vz -p by -p bz -t -1 \
#           cpaw_vlct_x_512.3d.xdmf

# plot movie:
# viscid_2d --own --slice z=0 -p vx -p vy -p vz -p bx -p by -p bz \
#           -a cpaw_vlct_x_512.mp4 --np 2 cpaw_vlct_x_512.3d.xdmf
