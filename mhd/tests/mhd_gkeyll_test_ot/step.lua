...
function runTimeStep(myDt, tCurr, step, cptr, ggcm_mhd)
   ...
   while true do
      ...
      if (useLaxSolver) then
        ...
        Lucee.logInfo (string.format(" Taking step %5d at time %6g with dt %g (using Lax solvers)", step, tCurr, myDt))
        status, dtSuggested = solveTwoFluidLaxSystem(tCurr, tCurr+myDt, ggcm_mhd)
        useLaxSolver = false
      else
        Lucee.logInfo (string.format(" Taking step %5d at time %6g with dt %g", step, tCurr, myDt))
      ...
      if (useLaxSolver == true) then
        ...
        Lucee.logInfo (string.format(" ** Negative pressure or density at %8g! Will retake step with Lax fluxes", tCurr+myDt))
        q:copy(qDup)
        qNew:copy(qNewDup)
