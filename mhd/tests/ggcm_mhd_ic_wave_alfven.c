...
ggcm_mhd_ic_wave_alfven_run(struct ggcm_mhd_ic *ic)
{
  ...
  mrc_f3_foreach(f3, ix, iy, iz, 1, 1) {
    ...
    if(strcmp(sub->pdim, "x") == 0){
      ...
      MRC_F3(f3, _RV1Y , ix, iy, iz) = sub->pert * sin(k * r[0]);
      MRC_F3(f3, _RV1Z , ix, iy, iz) = 0.0;
      MRC_F3(f3, _B1X  , ix, iy, iz) = sub->B0;
      MRC_F3(f3, _B1Y  , ix, iy, iz) = sub->pert * sin(k * r[0] );
      MRC_F3(f3, _B1Z  , ix, iy, iz) = 0.0;
      
      MRC_F3(f3, _UU1 , ix, iy, iz) = sub->p0 / (gamma - 1.f) +
	...
	.5f * (sqr(MRC_F3(f3, _B1X, ix, iy, iz)) +
      ...
      MRC_F3(f3, _RV1X , ix, iy, iz) = sub->pert*sin( 4. * M_PI * r[1] );
      MRC_F3(f3, _RV1Z , ix, iy, iz) = 0.0;
      MRC_F3(f3, _B1Y , ix, iy, iz) = sub->B0;
      MRC_F3(f3, _B1X , ix, iy, iz) = sub->pert*sin( 4. * M_PI * r[1] );
      MRC_F3(f3, _B1Z , ix, iy, iz) = 0.0;
	...
	.5f * (sqr(MRC_F3(f3, _B1X, ix, iy, iz)) +
      ...
      MRC_F3(f3, _RR1, ix, iy, iz) = sub->n0;
      MRC_F3(f3, _RV1Y , ix, iy, iz) = 0.0; 
      MRC_F3(f3, _RV1X , ix, iy, iz) = sub->pert*sin( 4. * M_PI * r[2] );
      MRC_F3(f3, _RV1Z , ix, iy, iz) = 0.0;
      MRC_F3(f3, _B1Z , ix, iy, iz) = sub->B0;
      MRC_F3(f3, _B1X , ix, iy, iz) = sub->pert*sin( 4. * M_PI * r[2] );
