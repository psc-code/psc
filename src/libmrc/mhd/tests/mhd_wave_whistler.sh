...
#PBS -j oe
#
# Arguments: ./mhd_wave_whistler.sh [pdim [mode_num [ncells [v_par]]]]
...
#
# Run Notes: Whistler wave (right hand polarized). Use pdim and mode_num to
...
run_name="whistler_${pdim}_${ncells}"
...
# viscid_ts --slice x=0,y=0,z=0 -p vy -p vz -p by -p bz whistler_x_128.3d.xdmf

# plot final time step:
# viscid_2d --own --slice z=0 -p vy -p vz -p by -p bz -t -1 whistler_x_128.3d.xdmf

# plot movie:
# viscid_2d --own --slice z=0 -p vx -p vy -p vz -p bx -p by -p bz \
#           -a whistler_x_128.mp4 --np 2 whistler_x_128.3d.xdmf
