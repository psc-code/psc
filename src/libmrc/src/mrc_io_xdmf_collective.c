...
static void xdmf_collective_setup(struct mrc_io* io)
{
  ...
  MPI_Comm_split(mrc_io_comm(io), xdmf->is_writer, io->rank,
                 &xdmf->comm_writers);
}
...
static void xdmf_collective_destroy(struct mrc_io* io)
{
  struct xdmf* xdmf = to_xdmf(io);
...
static void xdmf_collective_open(struct mrc_io* io, const char* mode)
{
  struct xdmf* xdmf = to_xdmf(io);
...
static void xdmf_collective_close(struct mrc_io* io)
{
  struct xdmf* xdmf = to_xdmf(io);
  struct xdmf_file* file = &xdmf->file;
...
static void xdmf_collective_write_attr(struct mrc_io* io, const char* path,
...
static void xdmf_collective_read_attr(struct mrc_io* io, const char* path,
                                      int type, const char* name,
                                      union param_u* pv)
{
  struct xdmf* xdmf = to_xdmf(io);
  ...
  switch (type) {
    case PT_SELECT:
    case PT_INT:
    case MRC_VAR_INT: MPI_Bcast(&pv->u_int, 1, MPI_INT, root, comm); break;
    case PT_BOOL:
    case MRC_VAR_BOOL:;
      int val = pv->u_int;
      ...
      pv->u_int = val;
      break;
    case PT_FLOAT: MPI_Bcast(&pv->u_float, 1, MPI_FLOAT, root, comm); break;
    case PT_DOUBLE: MPI_Bcast(&pv->u_double, 1, MPI_DOUBLE, root, comm); break;
    case PT_STRING:;
      int len;
      if (io->rank == root) {
      ...
      break;
    case PT_INT3: MPI_Bcast(pv->u_int3, 3, MPI_INT, root, comm); break;
    case PT_FLOAT3: MPI_Bcast(pv->u_float3, 3, MPI_FLOAT, root, comm); break;
    case PT_DOUBLE3: MPI_Bcast(pv->u_double3, 3, MPI_DOUBLE, root, comm); break;
    case PT_INT_ARRAY:
      ...
      MPI_Bcast(pv->u_int_array.vals, pv->u_int_array.nr_vals, MPI_INT, root,
                comm);
      break;
    case PT_PTR: break;
    default:
...
static void collective_m1_write_f1(struct mrc_io* io, const char* path,
                                   struct mrc_ndarray* nd1, int m,
                                   const char* name, hid_t group0)
{
  struct xdmf* xdmf = to_xdmf(io);
  int ierr;
  ...
  H5_CHK(group);
  int i0 = 0;
  ierr = H5LTset_attribute_int(group, ".", "global_patch", &i0, 1);
  CE;
  ...
  ierr = H5Dwrite(dset, H5T_NATIVE_FLOAT, memspace, filespace, dxpl, nd1->arr);
  CE;
...
static void collective_m1_send_begin(struct mrc_io* io,
...
static void xdmf_collective_write_m1(struct mrc_io* io, const char* path,
                                     struct mrc_fld* m1)
{
  struct xdmf* xdmf = to_xdmf(io);
  struct xdmf_file* file = &xdmf->file;
  ...
  if (xdmf->is_writer) {
    ...
    mrc_ndarray_set_param_int_array(nd1, "dims", 1,
                                    (int[1]){xdmf->slab_dims[dim]});
    mrc_ndarray_set_param_int_array(nd1, "offs", 1,
                                    (int[1]){xdmf->slab_off[dim]});
...
static void collective_m1_read_recv_begin(struct mrc_io* io,
                                          struct collective_m1_ctx* ctx,
                                          struct mrc_fld* m1, int m)
{
  struct xdmf* xdmf = to_xdmf(io);
...
static void collective_m1_read_recv_end(struct mrc_io* io,
                                        struct collective_m1_ctx* ctx,
                                        struct mrc_fld* m1, int m)
{
...
static void collective_m1_read_send_begin(struct mrc_io* io,
                                          struct collective_m1_ctx* ctx,
                                          struct mrc_domain* domain,
                                          struct mrc_fld* f1, int m)
{
  struct xdmf* xdmf = to_xdmf(io);
  ...
  for (int r = 0; r < io->size; r++) {
    MPI_Isend((char*)comp_name, strlen(comp_name) + 1, MPI_CHAR, r, 0,
              mrc_io_comm(io), &ctx->send_reqs[ctx->nr_send_reqs++]);
  }
  for (int gp = 0; gp < ctx->nr_global_patches; gp++) {
    struct mrc_patch_info info;
    mrc_domain_get_global_patch_info(domain, gp, &info);
    ...
    MPI_Isend(&MRC_F1(f1, m, ib), ie - ib, MPI_FLOAT, info.rank, gp,
              mrc_io_comm(io), &ctx->send_reqs[ctx->nr_send_reqs++]);
  }
}

static void collective_m1_read_send_end(struct mrc_io* io,
                                        struct collective_m1_ctx* ctx)
{
  struct xdmf* xdmf = to_xdmf(io);
...
static void xdmf_collective_read_m1(struct mrc_io* io, const char* path,
                                    struct mrc_fld* m1)
{
  struct xdmf* xdmf = to_xdmf(io);
  struct xdmf_file* file = &xdmf->file;
  ...
  if (xdmf->is_writer) {
    ...
    mrc_fld_set_param_int_array(f1, "dims", 2,
                                (int[2]){gdims[ctx.dim], nr_comps});
    ...
    hid_t memspace;
    if (io->rank == xdmf->writers[0]) {
      memspace = H5Screate_simple(1, hgdims, NULL);
      H5_CHK(memspace);
    } else {
      memspace = H5Screate(H5S_NULL);
      H5Sselect_none(memspace);
...
static void writer_write_fld(struct mrc_redist* redist, struct mrc_io* io,
                             const char* path, struct mrc_ndarray* nd, int m,
                             struct mrc_fld* m3, struct xdmf_spatial* xs,
                             hid_t group0)
{
  int ierr;
  ...
  hsize_t fdims[3] = {redist->slab_dims[2], redist->slab_dims[1],
                      redist->slab_dims[0]};
  ...
  hsize_t foff[3] = {ib[2] - redist->slab_offs[2], ib[1] - redist->slab_offs[1],
  ...
  ierr = H5Dwrite(dset, dtype, memspace, filespace, dxpl, nd->arr);
  CE;
...
static void xdmf_collective_write_m3(struct mrc_io* io, const char* path,
                                     struct mrc_fld* m3)
{
  struct xdmf* xdmf = to_xdmf(io);
  ...
  if (!xs) {
    xs = xdmf_spatial_create_m3_parallel(
      &file->xdmf_spatial_list, mrc_domain_name(m3->_domain), m3->_domain,
      redist->slab_offs, redist->slab_dims, io);
  }

  // If we have an aos field, we need to get it as soa for collection and
  // writing
...
static void collective_m3_init(struct mrc_io* io, struct collective_m3_ctx* ctx,
                               struct mrc_domain* domain)
{
  struct xdmf* xdmf = to_xdmf(io);
...
static void get_writer_off_dims(struct collective_m3_ctx* ctx, int writer,
                                int* writer_off, int* writer_dims)
{
  ...
  writer_dims[ctx->slow_dim] =
    ctx->slow_indices_per_writer + (writer < ctx->slow_indices_rmndr);
  if (writer < ctx->slow_indices_rmndr) {
    writer_off[ctx->slow_dim] += (ctx->slow_indices_per_writer + 1) * writer;
  } else {
    writer_off[ctx->slow_dim] +=
      ctx->slow_indices_rmndr + ctx->slow_indices_per_writer * writer;
  }
}

static void collective_m3_send_setup(struct mrc_io* io,
                                     ...
                                     struct mrc_fld* gfld)
{
  ...
  for (int i = 0, gp = 0; i < send->n; i++) {
    ...
    do {
      mrc_domain_get_global_patch_info(domain, gp++, &info);
      has_intersection =
        find_intersection(block->ilo, block->ihi, info.off, info.ldims,
                          mrc_fld_ghost_offs(gfld), mrc_fld_ghost_dims(gfld));
...
static void collective_m3_send_begin(struct mrc_io* io,
                                     struct collective_m3_ctx* ctx,
                                     struct mrc_domain* domain,
                                     struct mrc_fld* gfld)
{
...
static void collective_m3_send_end(struct mrc_io* io,
                                   struct collective_m3_ctx* ctx)
{
  struct mrc_redist_read* send = &ctx->read_send;
...
static void collective_m3_recv_setup(struct mrc_io* io,
                                     struct collective_m3_ctx* ctx,
                                     struct mrc_domain* domain)
{
  struct xdmf* xdmf = to_xdmf(io);
  ...
  for (int p = 0; p < ctx->nr_patches; p++) {
    ...
    for (int writer = 0; writer < xdmf->nr_writers; writer++) {
      ...
      int ilo[3], ihi[3];
      if (find_intersection(ilo, ihi, info.off, info.ldims, writer_off,
                            writer_dims)) {
        recv->n++;
      }
    }
...
static void collective_m3_recv_begin(struct mrc_io* io,
                                     struct collective_m3_ctx* ctx,
                                     struct mrc_domain* domain,
                                     struct mrc_fld* m3)
{
...
static void collective_m3_recv_end(struct mrc_io* io,
                                   struct collective_m3_ctx* ctx,
                                   struct mrc_domain* domain,
                                   struct mrc_fld* m3)
{
      ...
      case MRC_NT_FLOAT:
        for (int m = 0; m < mrc_fld_nr_comps(m3); m++) {
          for (int iz = ilo[2]; iz < ihi[2]; iz++) {
            for (int iy = ilo[1]; iy < ihi[1]; iy++) {
              for (int ix = ilo[0]; ix < ihi[0]; ix++) {
                MRC_S5((m3p)->_fld, ix, iy, iz, m, (m3p)->_p) =
                  MRC_S4(block->fld, ix, iy, iz, m);
              }
            }
          }
        }
      ...
      case MRC_NT_DOUBLE:
        for (int m = 0; m < mrc_fld_nr_comps(m3); m++) {
          for (int iz = ilo[2]; iz < ihi[2]; iz++) {
            for (int iy = ilo[1]; iy < ihi[1]; iy++) {
              for (int ix = ilo[0]; ix < ihi[0]; ix++) {
                MRC_D5((m3p)->_fld, ix, iy, iz, m, (m3p)->_p) =
                  MRC_D4(block->fld, ix, iy, iz, m);
              }
            }
          }
        }
      ...
      case MRC_NT_INT:
        for (int m = 0; m < mrc_fld_nr_comps(m3); m++) {
          for (int iz = ilo[2]; iz < ihi[2]; iz++) {
            for (int iy = ilo[1]; iy < ihi[1]; iy++) {
              for (int ix = ilo[0]; ix < ihi[0]; ix++) {
                MRC_I5((m3p)->_fld, ix, iy, iz, m, (m3p)->_p) =
                  MRC_I4(block->fld, ix, iy, iz, m);
              }
            }
          }
        }
                         ...
                         void* op_data)
{
  ...
  ierr = H5Dread(dset, dtype, data->memspace, data->filespace, data->dxpl, arr);
  CE;
  ierr = H5Dclose(dset);
  CE;
...
static void collective_m3_read_fld(struct mrc_io* io,
                                   struct collective_m3_ctx* ctx, hid_t group0,
                                   struct mrc_fld* fld)
{
  struct xdmf* xdmf = to_xdmf(io);
  int ierr;
  ...
  mrc_fld_set_param_int_array(
    fld, "dims", 4,
    (int[4]){writer_dims[0], writer_dims[1], writer_dims[2], nr_comps});
  mrc_fld_set_param_int_array(
    fld, "offs", 4, (int[4]){writer_off[0], writer_off[1], writer_off[2], 0});
  mrc_fld_setup(fld);
...
static void xdmf_collective_read_m3(struct mrc_io* io, const char* path,
                                    struct mrc_fld* m3)
{
  struct xdmf* xdmf = to_xdmf(io);
  struct xdmf_file* file = &xdmf->file;
  ...
  // If we have an aos field, we need to get it as soa for reading and
  // distribution
  struct mrc_fld* m3_soa = m3;
  if (m3->_aos) {
    switch (mrc_fld_data_type(m3)) {
