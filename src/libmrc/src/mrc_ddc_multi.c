...
static void mrc_ddc_multi_get_nei_rank_patch(struct mrc_ddc* ddc, int p,
                                             int dir[3], int* nei_rank,
                                             int* nei_patch)
{
...
static void ddc_init_outside(struct mrc_ddc* ddc, int p,
                             struct mrc_ddc_sendrecv* sr, int dir[3],
                             int ibn[3])
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
...
static void ddc_init_inside(struct mrc_ddc* ddc, int p,
                            struct mrc_ddc_sendrecv* sr, int dir[3], int ibn[3])
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
...
static void mrc_ddc_multi_set_domain(struct mrc_ddc* ddc,
...
static struct mrc_domain* mrc_ddc_multi_get_domain(struct mrc_ddc* ddc)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
...
static void mrc_ddc_multi_free_buffers(struct mrc_ddc* ddc,
...
static void mrc_ddc_multi_alloc_buffers(struct mrc_ddc* ddc,
                                        struct mrc_ddc_pattern2* patt2,
                                        int n_fields)
{
  if (ddc->size_of_type > patt2->max_size_of_type ||
    ...
    patt2->recv_buf =
      malloc(patt2->n_recv * patt2->max_n_fields * patt2->max_size_of_type);
    patt2->send_buf =
      malloc(patt2->n_send * patt2->max_n_fields * patt2->max_size_of_type);
    patt2->local_buf = malloc(patt2->local_buf_size * patt2->max_n_fields *
                              patt2->max_size_of_type);
  }
}
...
static void mrc_ddc_multi_setup_pattern2(
  struct mrc_ddc* ddc, struct mrc_ddc_pattern2* patt2,
  void (*init_send)(struct mrc_ddc*, int p, struct mrc_ddc_sendrecv*, int[3],
                    int[3]),
  void (*init_recv)(struct mrc_ddc*, int p, struct mrc_ddc_sendrecv*, int[3],
                    int[3]),
  int ibn[3])
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
  ...
  for (int r = 0; r < sub->mpi_size; r++) {
    if (ri[r].n_send_entries) {
      ri[r].send_entry =
        malloc(ri[r].n_send_entries * sizeof(*ri[r].send_entry));
    ...
    if (ri[r].n_recv_entries) {
      ...
      ri[r].recv_entry_ =
        malloc(ri[r].n_recv_entries * sizeof(*ri[r].recv_entry_));
  ...
  for (int i = 0; i < ri[sub->mpi_rank].n_recv_entries; i++) {
    ...
    local_buf_size = MAX(local_buf_size, (re->ihi[0] - re->ilo[0]) *
                                           (re->ihi[1] - re->ilo[1]) *
                                           (re->ihi[2] - re->ilo[2]));
  }
...
static void mrc_ddc_multi_destroy_pattern2(struct mrc_ddc* ddc,
                                           struct mrc_ddc_pattern2* patt2)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
...
static void mrc_ddc_multi_set_mpi_type(struct mrc_ddc* ddc)
{
...
static void mrc_ddc_multi_setup(struct mrc_ddc* ddc)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
  ...
  sub->patches = mrc_domain_get_patches(sub->domain, &sub->nr_patches);

  MPI_Comm_rank(mrc_ddc_comm(ddc), &sub->mpi_rank);
  MPI_Comm_size(mrc_ddc_comm(ddc), &sub->mpi_size);

  mrc_ddc_multi_setup_pattern2(ddc, &sub->fill_ghosts2, ddc_init_inside,
                               ddc_init_outside, ddc->ibn);
  mrc_ddc_multi_setup_pattern2(ddc, &sub->add_ghosts2, ddc_init_outside,
                               ddc_init_inside, ddc->ibn);
  prof_stop(pr);
}
...
static void mrc_ddc_multi_destroy(struct mrc_ddc* ddc)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
...
static void ddc_run_begin(struct mrc_ddc* ddc, struct mrc_ddc_pattern2* patt2,
                          int mb, int me, void* ctx,
                          void (*to_buf)(int mb, int me, int p, int ilo[3],
                                         int ihi[3], void* buf, void* ctx))
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
...
static void ddc_run_end(struct mrc_ddc* ddc, struct mrc_ddc_pattern2* patt2,
                        int mb, int me, void* ctx,
                        void (*from_buf)(int mb, int me, int p, int ilo[3],
                                         int ihi[3], void* buf, void* ctx))
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
  struct mrc_ddc_rank_info* ri = patt2->ri;
...
static void ddc_run_local(struct mrc_ddc* ddc, struct mrc_ddc_pattern2* patt2,
                          int mb, int me, void* ctx,
                          void (*to_buf)(int mb, int me, int p, int ilo[3],
                                         int ihi[3], void* buf, void* ctx),
                          void (*from_buf)(int mb, int me, int p, int ilo[3],
...
static void ddc_run(struct mrc_ddc* ddc, struct mrc_ddc_pattern2* patt2, int mb,
                    int me, void* ctx,
                    void (*to_buf)(int mb, int me, int p, int ilo[3],
                                   int ihi[3], void* buf, void* ctx),
                    void (*from_buf)(int mb, int me, int p, int ilo[3],
...
static void mrc_ddc_multi_add_ghosts(struct mrc_ddc* ddc, int mb, int me,
                                     void* ctx)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
  ...
  ddc_run(ddc, &sub->add_ghosts2, mb, me, ctx, ddc->funcs->copy_to_buf,
          ddc->funcs->add_from_buf);
}

// ----------------------------------------------------------------------
// mrc_ddc_multi_fill_ghosts

static void mrc_ddc_multi_fill_ghosts(struct mrc_ddc* ddc, int mb, int me,
                                      void* ctx)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
  ...
  ddc_run(ddc, &sub->fill_ghosts2, mb, me, ctx, ddc->funcs->copy_to_buf,
...
static void mrc_ddc_multi_fill_ghosts_begin(struct mrc_ddc* ddc, int mb, int me,
                                            void* ctx)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
  ...
  mrc_ddc_multi_set_mpi_type(ddc);
  mrc_ddc_multi_alloc_buffers(ddc, &sub->fill_ghosts2, me - mb);
  ddc_run_begin(ddc, &sub->fill_ghosts2, mb, me, ctx, ddc->funcs->copy_to_buf);
}

static void mrc_ddc_multi_fill_ghosts_end(struct mrc_ddc* ddc, int mb, int me,
                                          void* ctx)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);

  ddc_run_end(ddc, &sub->fill_ghosts2, mb, me, ctx, ddc->funcs->copy_from_buf);
}

static void mrc_ddc_multi_fill_ghosts_local(struct mrc_ddc* ddc, int mb, int me,
                                            void* ctx)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);

  ddc_run_local(ddc, &sub->fill_ghosts2, mb, me, ctx, ddc->funcs->copy_to_buf,
                ddc->funcs->copy_from_buf);
}
...
static void mrc_ddc_multi_fill_ghosts_fld(struct mrc_ddc* ddc, int mb, int me,
                                          struct mrc_fld* fld)
{
  struct mrc_ddc_multi* sub = mrc_ddc_multi(ddc);
