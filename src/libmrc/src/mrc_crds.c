...
static void _mrc_crds_create(struct mrc_crds* crds)
{
  for (int d = 0; d < 3; d++) {
    char s[20];
    ...
    // These aren't listed as member objects, so we need to create them
    // ourselves here because we do something hacky when writing them for
    // xdmf_collective
...
static void _mrc_crds_read(struct mrc_crds* crds, struct mrc_io* io)
{
  if (strcmp(mrc_io_type(io), "hdf5_serial") != 0 &&
      strcmp(mrc_io_type(io), "xdmf_serial") != 0) {
    // FIXME, but reading back coordinates is broken for everything but
    // hdf5_serial, because for other mrc_io types, we don't write crd_nc (at
    // least not completely), so there's no (easy) way to restore it.
    assert(0);
  }
  ...
  if (strcmp(mrc_io_type(io), "hdf5_serial") == 0) {
    ...
    crds->global_crd[0] =
      mrc_io_read_ref(io, crds, "global_crd[0]", mrc_ndarray);
    crds->global_crd[1] =
      mrc_io_read_ref(io, crds, "global_crd[1]", mrc_ndarray);
    crds->global_crd[2] =
      mrc_io_read_ref(io, crds, "global_crd[2]", mrc_ndarray);
  }
...
static void _mrc_crds_write(struct mrc_crds* crds, struct mrc_io* io)
{
  // FIXME, hacky to no end...
  // So for xdmf_collective, we need to write crd_nc while chopping of ghost
  // points, which breaks completely reading them back for
  // checkpointing/hdf5_serial, however, we write them including ghost points,
  // just like every other coordinate field...
  // ...except that every other coordinate field is listed as a member object,
  // so it happens automatically, but here we need to do it explicitly.
...
void mrc_crds_get_dx_base(struct mrc_crds* crds, double dx[3])
{
  ...
  // the only place where this makes sense is if we have one patch / proc, no
  // AMR, anyway
  mrc_crds_get_dx(crds, 0, dx); // the only
}

// allocate the coordinate fields common to all crds types.
static void mrc_crds_setup_alloc_only(struct mrc_crds* crds)
{
...
static void mrc_crds_setup_alloc_global_array(struct mrc_crds* crds)
{
  int gdims[3];
  mrc_domain_get_global_dims(crds->domain, gdims);
  ...
  for (int d = 0; d < 3; d++) {
    ...
    mrc_ndarray_set_param_int_array(crds->global_crd[d], "dims", 2,
                                    (int[2]){gdims[d] + 2 * crds->sw, 2});
    mrc_ndarray_set_param_int_array(crds->global_crd[d], "offs", 2,
                                    (int[2]){-crds->sw, 0});
...
static void _mrc_crds_setup(struct mrc_crds* crds)
{
  int gdims[3];
  int nr_patches;
...
static void _mrc_crds_destroy(struct mrc_crds* crds)
{
  for (int d = 0; d < 3; d++) {
...
static void mrc_crds_uniform_setup(struct mrc_crds* crds)
{
  for (int d = 0; d < 3; d++) {
...
static void mrc_crds_mb_create(struct mrc_crds* crds)
{
  // Name the standard 3 crds_gen to "UNUSED" to (hopefully) avoid some
  // confusion
  for (int d = 0; d < 3; d++) {
...
static void mrc_crds_mb_read(struct mrc_crds* crds, struct mrc_io* io)
{
...
static void mrc_crds_mb_setup(struct mrc_crds* crds)
{
  // this should still work
  mrc_crds_setup_alloc_only(crds);

  typedef void (*dgb_t)(struct mrc_domain*, struct MB_block** pblock,
                        int* nr_blocks);
  dgb_t domain_get_blocks =
    (dgb_t)mrc_domain_get_method(crds->domain, "get_blocks");
  ...
  for (int b = 0; b < nr_blocks; b++) {
    ...
    for (int d = 0; d < 3; d++) {
      ...
      // If I had my way, I'd kill off the original crds_gen children to
      // minimize confusion, but I guess I'll just have to write the docs to
