		...
		const char *name, union param_u *pv)
{
  ...
  case PT_PTR:
    break;
  default:
    mprintf("mrc_io_hdf5_parallel: not writing attr '%s' (type %d)\n",
	    name, type);
    assert(0);
  }
		...
		const char *name, union param_u *pv)
{
  ...
  case PT_PTR:
    break;
  default:
    mprintf("mrc_io_hdf5_parallel: not reading attr '%s' (type %d)\n", name, type);
    assert(0);
    break;
  }
  ierr = H5Gclose(group); CE;
...
hdf5_parallel_write_fld(struct mrc_io *io, const char *path, struct mrc_fld *fld)
{
  ...
  H5Dwrite(dset, datatype, memspace, filespace, dxpl,
	   fld->_arr);
...
hdf5_parallel_read_fld(struct mrc_io *io, const char *path, struct mrc_fld *fld)
{
  ...
  H5Dread(dset, datatype, memspace, filespace, dxpl,
	   fld->_arr);
