...
// cause instantaneous changes in electric potential/field everywhere,
// information propagates at the speed of light (which is a parameter)
...
static void psc_es1_create(struct psc* psc)
{
...
static double psc_es1_init_field(struct psc* psc, double x[3], int m)
{
...
static void psc_es1_init_species(struct psc* psc, int kind,
                                 struct psc_es1_species* s,
...
void psc_es1_setup_particles(struct psc* psc, int* nr_particles_by_patch,
                             bool count_only)
{
  struct psc_es1* es1 = to_psc_es1(psc);
  ...
  psc_foreach_patch(psc, p)
  {
    int il1 = 0;
    for (int kind = 0; kind < psc->nr_kinds; kind++) {
      psc_es1_init_species(psc, kind, &es1->species[kind], mprts.mprts(), p,
                           &il1);
    }
  }
