...
static void psc_diag_item_mirror_run(struct psc_diag_item* item,
                                     struct psc* psc, double* result)
{
  ...
  psc_foreach_patch(psc, p)
  {
    ...
    {
      result[0] +=
        (sqr(F(HX, ix, iy, iz) - HX0) + sqr(F(HY, ix, iy, iz) - HY0) +
         sqr(F(HZ, ix, iy, iz) - HZ0)) *
        fac;
    }
    foreach_3d_end;
  }
...
static void psc_mirror_create(struct psc* psc)
{
...
static void psc_mirror_init_npt(struct psc* psc, int kind, double x[3],
...
static double psc_mirror_init_field(struct psc* psc, double x[3], int m)
{
  struct psc_mirror* mirror = to_psc_mirror(psc);
  double B0 = mirror->vA_over_c;
  double B1 = mirror->B1;
  ...
  switch (m) {
    case HZ: return B0 + B1 * cos(ky * y + kz * z);
...
static void psc_mirror_setup(struct psc* psc)
{
  struct psc_mirror* mirror = to_psc_mirror(psc);
...
struct psc_ops psc_mirror_ops = {.name = "mirror",
