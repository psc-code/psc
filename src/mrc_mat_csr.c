...
// ----------------------------------------------------------------------
// mrc_mat_csr_add_empty_rows_if_needed
//
...
static void
_mrc_mat_csr_expand_nr_rows_if_needed(struct mrc_mat *mat, int nr_rows)
{
  struct mrc_mat_csr *sub = mrc_mat_csr(mat);
...
mrc_mat_csr_add_value(struct mrc_mat *mat, int row_idx, int col_idx, double val)
{
  ...
  _mrc_mat_csr_expand_nr_rows_if_needed(mat, row_idx + 1);
...
mrc_mat_csr_assemble(struct mrc_mat *mat)
{
  ...
  _mrc_mat_csr_expand_nr_rows_if_needed(mat, sub->nr_rows);
