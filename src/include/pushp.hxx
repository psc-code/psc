...
struct AdvanceParticle
{
  ...
  __host__ __device__ AdvanceParticle(real_t dt) : dt_(dt) {}

  // ----------------------------------------------------------------------
  // push_x

  __host__ __device__ inline void push_x(Real3& x, const Real3& v,
                                         real_t dt_fac = real_t{1.})
  {
    if (!dim::InvarX::value) {
      x[0] += dt_fac * dt_ * v[0];
    }
  ...
  __host__ __device__ inline void push_p(Real3& p, const Real3& E,
                                         const Real3& H, real_t dq)
  {
