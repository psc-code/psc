...
#define CRDX(p, jx) (psc->grid_->domain.dx[0] * (jx) + psc->grid_->patches[p].xb[0])
#define CRDY(p, jy) (psc->grid_->domain.dx[1] * (jy) + psc->grid_->patches[p].xb[1])
#define CRDZ(p, jz) (psc->grid_->domain.dx[2] * (jz) + psc->grid_->patches[p].xb[2])
