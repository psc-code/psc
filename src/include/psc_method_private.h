...
struct psc_method_ops {
  ...
  std::vector<uint> (*setup_partition)(struct psc_method *method, struct psc *psc);
  void (*set_ic_particles)(struct psc_method *method, struct psc *psc, std::vector<uint>& n_prts_by_patch);
