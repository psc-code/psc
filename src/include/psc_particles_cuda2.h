...
#define psc_particles_cuda2(prts)                                              \
  mrc_to_subobj(prts, struct psc_particles_cuda2)
...
#define psc_mparticles_cuda2(prts)                                             \
  mrc_to_subobj(prts, struct psc_mparticles_cuda2)

#define PARTICLE_CUDA2_LOAD_POS(prt, d_xi4, n)                                 \
  do {                                                                         \
    float4 xi4 = d_xi4[n];                                                     \
    (prt).xi[0] = xi4.x;                                                       \
    (prt).xi[1] = xi4.y;                                                       \
...
#define PARTICLE_CUDA2_LOAD_MOM(prt, d_pxi4, n)                                \
  do {                                                                         \
...
#define PARTICLE_CUDA2_STORE_POS(pp, d_xi4, n)                                 \
  do {                                                                         \
    float4 xi4 = {(pp).xi[0], (pp).xi[1], (pp).xi[2], (pp).kind_as_float};     \
    d_xi4[n] = xi4;                                                            \
  } while (0)

#define PARTICLE_CUDA2_STORE_MOM(pp, d_pxi4, n)                                \
  do {                                                                         \
