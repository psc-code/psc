...
void initializeAlfven(MfieldsAlfven& mflds){
  ...
  double rand_ph[8]={2. * M_PI * Rng_uniform(rng, rph_a, rph_b), // I think this numbers need to change at each time
                    ...
                    2. * M_PI * Rng_uniform(rng, rph_a, rph_b),
                    2. * M_PI * Rng_uniform(rng, rph_a, rph_b)};
  dcomp unk[8]={ Rng_uniform(rng, ua, ub) + 2.i * M_PI * Rng_uniform(rng, ua, ub), // This needs to change at each time
               Rng_uniform(rng, ua, ub) + 2.i * M_PI * Rng_uniform(rng, ua, ub),
               Rng_uniform(rng, ua, ub) + 2.i * M_PI * Rng_uniform(rng, ua, ub),
               Rng_uniform(rng, ua, ub) + 2.i * M_PI * Rng_uniform(rng, ua, ub),
               Rng_uniform(rng, ua, ub) + 2.i * M_PI * Rng_uniform(rng, ua, ub),
               Rng_uniform(rng, ua, ub) + 2.i * M_PI * Rng_uniform(rng, ua, ub),
...
void initializeFields(MfieldsState& mflds, MfieldsAlfven& mflds_alfven)
{
  setupFieldsGeneral(
    mflds, [&](int m, Int3 idx, int p, double crd[3]) -> MfieldsState::real_t {
      ...
      case HX:
        return 0.;//mflds_alfven(PERT_VX, idx[0], idx[1], idx[2], p); //0. ; 
      case HY:
        return  0. + g.dby * sin(2. * M_PI * (z - 0.5 * g.Lz) / g.Lz) * cos(M_PI * y / g.Ly); // + dB_azT //In  the case of yz geometry
      case HZ:
        return g.b0 * tanh(y / g.L) + g.dbz * cos(2. * M_PI * (z - 0.5 * g.Lz) / g.Lz) * sin(M_PI * y / g.Ly);
      //***/
      //--------------------------------------------------------------------------------  
