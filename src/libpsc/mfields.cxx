...
void MfieldsBase::convert(MfieldsBase& mf_from, MfieldsBase& mf_to, int mb,
                          int me)
{
  ...
  auto convert_from =
    mf_to.convert_from().find(std::type_index(typeid(mf_from)));
...
void MfieldsStateBase::convert(MfieldsStateBase& mf_from,
                               MfieldsStateBase& mf_to, int mb, int me)
{
  // FIXME, implementing == wouldn't hurt
  assert(&mf_from._grid() == &mf_to._grid());
