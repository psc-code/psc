...
public:
                  ...
                  size_t n_total, const particles_type& arr, int timestep)
  {
    ...
    // truncated to e.g. prt.1234567890_p000000.h (with .h instead of .h5)

    // This is probably a problem with other outputs, too
    int slen = strlen(params_.data_dir) + strlen(params_.basename) + 23;
    char filename[slen];
    ...
    snprintf(filename, slen, "%s/%s.%09d_p%06d.h5", params_.data_dir,
