			      ...
			      const char *pfx)
{
  ...
  fields_c_t *fld = psc_mfields_c_get_patch_c(list->flds[0], 0);
  ...
  for (int kz = 0; kz < np[2]; kz++) {
	...
	for (int m = 0; m < list->nr_flds; m++) {
	  fld = psc_mfields_c_get_patch_c(list->flds[m], 0);
				    ...
				    const char *pfx)
{
  ...
  for (int m = 0; m < list->nr_flds; m++) {
    hdf5_write_field(&hdf5, psc_mfields_c_get_patch_c(list->flds[m], 0));
  }
  hdf5_close(&hdf5);
