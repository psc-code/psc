...
static void psc_fields_cuda2_zero_comp(struct psc_fields* pf, int m)
{
...
static void psc_fields_cuda2_copy_from_single(struct psc_fields* flds_cuda2,
                                              struct psc_fields* flds_single,
                                              int mb, int me)
{
  for (int m = mb; m < me; m++) {
    for (int jz = flds_cuda2->ib[2]; jz < flds_cuda2->ib[2] + flds_cuda2->im[2];
         jz++) {
      for (int jy = flds_cuda2->ib[1];
           jy < flds_cuda2->ib[1] + flds_cuda2->im[1]; jy++) {
        for (int jx = flds_cuda2->ib[0];
             jx < flds_cuda2->ib[0] + flds_cuda2->im[0]; jx++) {
          F3_CUDA2(flds_cuda2, m, jx, jy, jz) =
            F3_S(flds_single, m, jx, jy, jz);
        }
      }
    }
  }
...
static void psc_fields_cuda2_copy_to_single(struct psc_fields* flds_cuda2,
                                            struct psc_fields* flds_single,
                                            int mb, int me)
{
...
EXTERN_C void __fields_cuda_to_device(struct psc_fields* pf, real* h_flds,
                                      int mb, int me);
EXTERN_C void __fields_cuda_from_device(struct psc_fields* pf, real* h_flds,
                                        int mb, int me);

static void psc_fields_cuda2_copy_from_cuda(struct psc_fields* flds_cuda2,
                                            struct psc_fields* flds_cuda,
                                            int mb, int me)
{
  float* h_flds =
    malloc(flds_cuda->nr_comp * psc_fields_size(flds_cuda) * sizeof(*h_flds));

  __fields_cuda_from_device(flds_cuda, h_flds, mb, me);

  for (int m = mb; m < me; m++) {
    for (int jz = flds_cuda->ib[2]; jz < flds_cuda->ib[2] + flds_cuda->im[2];
         jz++) {
      for (int jy = flds_cuda->ib[1]; jy < flds_cuda->ib[1] + flds_cuda->im[1];
           jy++) {
        for (int jx = flds_cuda->ib[0];
             jx < flds_cuda->ib[0] + flds_cuda->im[0]; jx++) {
          F3_CUDA2(flds_cuda2, m, jx, jy, jz) =
            F3_CUDA(flds_cuda, m, jx, jy, jz);
        }
      }
    }
  }
...
static void psc_fields_cuda2_copy_to_cuda(struct psc_fields* flds_cuda2,
                                          struct psc_fields* flds_cuda, int mb,
                                          int me)
{
  float* h_flds =
    malloc(flds_cuda->nr_comp * psc_fields_size(flds_cuda) * sizeof(*h_flds));
...
static void psc_mfields_cuda2_setup(struct psc_mfields* mflds)
{
  ...
  sub->d_flds =
    cuda_calloc(mflds->nr_fields * total_size, sizeof(*sub->d_flds));
}

// ----------------------------------------------------------------------
// psc_mfields_cuda2_destroy

static void psc_mfields_cuda2_destroy(struct psc_mfields* mflds)
{
  struct psc_mfields_cuda2* sub = psc_mfields_cuda2(mflds);
...
void psc_mfields_cuda2_copy_to_device(struct psc_mfields* mflds)
{
  struct psc_mfields_cuda2* sub = psc_mfields_cuda2(mflds);

  size_t total_size =
    sub->im[0] * sub->im[1] * sub->im[2] * mflds->nr_patches * mflds->nr_fields;
  cuda_memcpy_device_from_host(sub->d_flds, sub->h_flds,
                               total_size * sizeof(*sub->d_flds));
}

// ----------------------------------------------------------------------
// psc_mfields_cuda2_copy_to_host

void psc_mfields_cuda2_copy_to_host(struct psc_mfields* mflds)
{
  struct psc_mfields_cuda2* sub = psc_mfields_cuda2(mflds);

  size_t total_size =
    sub->im[0] * sub->im[1] * sub->im[2] * mflds->nr_patches * mflds->nr_fields;
  cuda_memcpy_host_from_device(sub->h_flds, sub->d_flds,
                               total_size * sizeof(*sub->d_flds));
}
