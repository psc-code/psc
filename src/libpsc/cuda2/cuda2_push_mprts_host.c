...
#define INTERPOLATE_1ST_EC(pf, exq, eyq, ezq, hxq, hyq, hzq)        	\
  do {									\
    ...
    particle_real_t g1z = og[2];					\
    									\
    exq = (g0z*(g0y*F3_S(pf, EX, 0,lg[1]  ,lg[2]  ) +		\
		g1y*F3_S(pf, EX, 0,lg[1]+1,lg[2]  )) +		\
	   g1z*(g0y*F3_S(pf, EX, 0,lg[1]  ,lg[2]+1) +		\
		g1y*F3_S(pf, EX, 0,lg[1]+1,lg[2]+1)));		\
    eyq = (g0z*F3_S(pf, EY, 0,lg[1]  ,lg[2]  ) +			\
	   g1z*F3_S(pf, EY, 0,lg[1]  ,lg[2]+1));			\
    ezq = (g0y*F3_S(pf, EZ, 0,lg[1]  ,lg[2]  ) +			\
	   g1y*F3_S(pf, EZ, 0,lg[1]+1,lg[2]  ));			\
									\
    hxq = F3_S(pf, HX, 0,lg[1]  ,lg[2]  );				\
    hyq = (g0y*F3_S(pf, HY, 0,lg[1]  ,lg[2]  ) +			\
	   g1y*F3_S(pf, HY, 0,lg[1]+1,lg[2]  ));			\
    hzq = (g0z*F3_S(pf, HZ, 0,lg[1]  ,lg[2]  ) +			\
	   g1z*F3_S(pf, HZ, 0,lg[1]  ,lg[2]+1));			\
    									\
		...
		particle_real_t fnq[3], particle_real_t dxt[3], int off[3])
{
  ...
  particle_real_t xa[3] = { 0.,
			    x[1] + .5f * dx[1],
			    x[2] + .5f * dx[2], };
  F3_S(pf, JXI, 0,i[1]  ,i[2]  ) += fnq[0] * (dx[0] * (.5f - xa[1]) * (.5f - xa[2]) + h);
  F3_S(pf, JXI, 0,i[1]+1,i[2]  ) += fnq[0] * (dx[0] * (.5f + xa[1]) * (.5f - xa[2]) - h);
  F3_S(pf, JXI, 0,i[1]  ,i[2]+1) += fnq[0] * (dx[0] * (.5f - xa[1]) * (.5f + xa[2]) + h);
  F3_S(pf, JXI, 0,i[1]+1,i[2]+1) += fnq[0] * (dx[0] * (.5f + xa[1]) * (.5f + xa[2]) - h);

  F3_S(pf, JYI, 0,i[1]  ,i[2]  ) += fnq[1] * dx[1] * (.5f - xa[2]);
  F3_S(pf, JYI, 0,i[1]  ,i[2]+1) += fnq[1] * dx[1] * (.5f + xa[2]);
  F3_S(pf, JZI, 0,i[1]  ,i[2]  ) += fnq[2] * dx[2] * (.5f - xa[1]);
  F3_S(pf, JZI, 0,i[1]+1,i[2]  ) += fnq[2] * dx[2] * (.5f + xa[1]);
  if (dxt) {
    dxt[0] -= dx[0];
    dxt[1] -= dx[1];
    dxt[2] -= dx[2];
...
static inline void
calc_jxyz_3d(struct psc_fields *flds, particle_real_t *xm, particle_real_t *xp,
