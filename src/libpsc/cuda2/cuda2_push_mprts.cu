...
CUDA_DEVICE static int find_patch()
{
...
CUDA_DEVICE static void find_ci0(int* ci0)
{
...
CUDA_GLOBAL static void CUDA_LAUNCH_BOUNDS(THREADS_PER_BLOCK, 3)
  push_mprts_ab(mprts_array_t mprts_arr, unsigned int* b_off, float* d_flds0,
                unsigned int size)
{
  ...
  CUDA_SYNCTHREADS();
  for (int n = (block_begin & ~31) + threadIdx.x; n < block_end;
       n += THREADS_PER_BLOCK) {
    if (n < block_begin) {
      continue;
    }
...
static void zero_currents(struct psc_mfields* mflds)
{
  ...
  for (int p = 0; p < mflds->nr_patches; p++) {
    fields_cuda2_real_t* d_flds =
      mflds_sub->d_flds + p * size * mflds->nr_fields;
...
static void push_mprts_loop(struct psc_mparticles* mprts,
                            struct psc_mfields* mflds)
{
  ...
  unsigned int fld_size =
    mflds->nr_fields * mflds_sub->im[0] * mflds_sub->im[1] * mflds_sub->im[2];

#ifdef __CUDACC__
  dim3 dimGrid(mprts_sub->b_mx[0], mprts_sub->b_mx[1],
               mprts_sub->b_mx[2] * mprts->nr_patches);

  mprts_array_t mprts_arr = {
    .xi4 = mprts_sub->d_xi4,
    .pxi4 = mprts_sub->d_pxi4,
  };

  push_mprts_ab<<<dimGrid, THREADS_PER_BLOCK>>>(mprts_arr, mprts_sub->d_b_off,
                                                mflds_sub->d_flds, fld_size);

  cuda_sync_if_enabled();
#else
  int dimGrid[3] = {mprts_sub->b_mx[0], mprts_sub->b_mx[1],
                    mprts_sub->b_mx[2] * mprts->nr_patches};

  mprts_array_t mprts_arr = {
    .xi4 = mprts_sub->h_xi4,
    .pxi4 = mprts_sub->h_pxi4,
  };
    ...
    for (blockIdx.y = 0; blockIdx.y < dimGrid[1]; blockIdx.y++) {
      for (blockIdx.x = 0; blockIdx.x < dimGrid[0]; blockIdx.x++) {
        for (threadIdx.x = 0; threadIdx.x < THREADS_PER_BLOCK; threadIdx.x++) {
          push_mprts_ab(mprts_arr, mprts_sub->h_b_off, mflds_sub->h_flds,
                        fld_size);
        }
      }
    }
...
void SFX(cuda2_1vbec_push_mprts)(struct psc_mparticles* mprts,
                                 struct psc_mfields* mflds)
{
  c_prm_set(ppsc);
