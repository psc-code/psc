...
static void PFX(write)(struct psc_mparticles* mprts, struct mrc_io* io)
{
  int ierr;
  ...
  hid_t group = H5Gopen(h5_file, mrc_io_obj_path(io, mprts), H5P_DEFAULT);
  H5_CHK(group);
  for (int p = 0; p < mprts->nr_patches; p++) {
    ...
    sprintf(pname, "p%d", p);
    hid_t pgroup =
      H5Gcreate(group, pname, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    H5_CHK(pgroup);
    int n_prts = prts.size();
    ierr = H5LTset_attribute_int(pgroup, ".", "n_prts", &n_prts, 1);
    CE;
    if (n_prts > 0) {
...
static void PFX(read)(struct psc_mparticles* mprts, struct mrc_io* io)
{
  int ierr;
  ...
  for (int p = 0; p < mprts->nr_patches; p++) {
    ...
    char pname[10];
    sprintf(pname, "p%d", p);
    hid_t pgroup = H5Gopen(group, pname, H5P_DEFAULT);
    H5_CHK(pgroup);
    int n_prts;
    ierr = H5LTget_attribute_int(pgroup, ".", "n_prts", &n_prts);
    CE;
    mparticles_t(mprts)[p].reserve(n_prts);

    if (n_prts > 0) {
#if PSC_PARTICLES_AS_SINGLE
      ierr = H5LTread_dataset_float(pgroup, "data", (float*)&*prts.begin());
      CE;
#elif PSC_PARTICLES_AS_DOUBLE
      ierr = H5LTread_dataset_double(pgroup, "data", (double*)&*prts.begin());
      CE;
#else
      assert(0);
#endif
