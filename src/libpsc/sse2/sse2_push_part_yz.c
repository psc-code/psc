...
static inline void push_xi_halfdt(struct particle_vec* p, pvReal* vyi,
                                  pvReal* vzi, pvReal* yl, pvReal* zl)
{
  pvReal tmpy, tmpz;
...
static void do_push_part_yz_a(particles_sse2_t* pp)
{
  //-----------------------------------------------------
...
static void do_push_part_yz_b(particles_sse2_t* pp, fields_sse2_t* pf)
{
  //-----------------------------------------------------
  // Initialization stuff
  ...
  //    sse2_real * restrict EXpoint = &pf->flds[EX*psc.fld_size + psc.ilo[0] -
  //    psc.ilg[0]]; sse2_real * restrict EYpoint = &pf->flds[EY*psc.fld_size +
  //    psc.ilo[0] - psc.ilg[0]]; sse2_real * restrict EZpoint =
  //    &pf->flds[EZ*psc.fld_size + psc.ilo[0] - psc.ilg[0]]; sse2_real *
  //    restrict HXpoint = &pf->flds[HX*psc.fld_size + psc.ilo[0] - psc.ilg[0]];
  //    sse2_real * restrict HYpoint = &pf->flds[HY*psc.fld_size + psc.ilo[0] -
  //    psc.ilg[0]]; sse2_real * restrict HZpoint = &pf->flds[HZ*psc.fld_size +
  //    psc.ilo[0] - psc.ilg[0]];
  ...
  for (int n = 0; n < pp->n_part; n += VEC_SIZE) {
    ...
    pvReal gmy, gmz, gOy, gOz, gly,
      glz; // NB: this is gO as in octogon instead of g0,
    ...
    pvReal hmy, hmz, hOy, hOz, hly,
      hlz; // NB: this is hO as in octogon instead of h0,
...
/// yz particle pushers
static void do_push_part_yz(particles_sse2_t* pp, fields_sse2_t* pf)
{
  //-----------------------------------------------------
...
#define JSX(indx2, indx3)                                                      \
  s_jxi[((indx3) + psc.ibn[2]) * (psc.patch[0].ldims[1] + 2 * psc.ibn[1]) +    \
        ((indx2) + psc.ibn[1])]
#define JSY(indx2, indx3)                                                      \
  s_jyi[((indx3) + psc.ibn[2]) * (psc.patch[0].ldims[1] + 2 * psc.ibn[1]) +    \
        ((indx2) + psc.ibn[1])]
#define JSZ(indx2, indx3)                                                      \
  s_jzi[((indx3) + psc.ibn[2]) * (psc.patch[0].ldims[1] + 2 * psc.ibn[1]) +    \
        ((indx2) + psc.ibn[1])]
  ...
  for (int n = 0; n < pp->n_part; n += VEC_SIZE) {
    ...
    for (int l3i = l3min; l3i < l3max; l3i++) {
      ...
      for (int l2i = l2min; l2i < l2max; l2i++) {
        ...
        for (int m = 0; m < VEC_SIZE; m++) {
          JSX(j2.v[m] + l2i - 2, j3.v[m] + l3i - 2) +=
            wx.v[m]; // undo index adjustment of l(2,3)i
          JSY(j2.v[m] + l2i - 2, j3.v[m] + l3i - 2) +=
            jyh.v[m]; // undo index adjustment of l(2,3)i
          JSZ(j2.v[m] + l2i - 2, j3.v[m] + l3i - 2) +=
            jzh[l2i].v[m]; // undo index adjustment of l(2
        }
      }
    }
  }
