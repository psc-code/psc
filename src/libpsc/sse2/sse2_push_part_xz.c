...
static inline void push_xi_halfdt(struct particle_vec* p, pvReal* vxi,
                                  pvReal* vzi, pvReal* xl, pvReal* zl)
{
  pvReal tmpx, tmpz;
...
/// xz particle pusher
static void do_push_part_xz(particles_sse2_t* pp, fields_sse2_t* pf)
{
...
#define JSX(indx1, indx3)                                                      \
  s_jxi[((indx3)-psc.ilg[2]) * psc.img[0] + ((indx1)-psc.ilg[0])]
#define JSY(indx1, indx3)                                                      \
  s_jyi[((indx3)-psc.ilg[2]) * psc.img[0] + ((indx1)-psc.ilg[0])]
#define JSZ(indx1, indx3)                                                      \
  s_jzi[((indx3)-psc.ilg[2]) * psc.img[0] + ((indx1)-psc.ilg[0])]
  ...
  sse2_real* restrict EXpoint =
    &pf->flds[EX * psc.fld_size + (psc.ilo[1] - psc.ilg[1]) * psc.img[0]];

  sse2_real* restrict EYpoint =
    &pf->flds[EY * psc.fld_size + (psc.ilo[1] - psc.ilg[1]) * psc.img[0]];

  sse2_real* restrict EZpoint =
    &pf->flds[EZ * psc.fld_size + (psc.ilo[1] - psc.ilg[1]) * psc.img[0]];

  sse2_real* restrict HXpoint =
    &pf->flds[HX * psc.fld_size + (psc.ilo[1] - psc.ilg[1]) * psc.img[0]];

  sse2_real* restrict HYpoint =
    &pf->flds[HY * psc.fld_size + (psc.ilo[1] - psc.ilg[1]) * psc.img[0]];

  sse2_real* restrict HZpoint =
    &pf->flds[HZ * psc.fld_size + (psc.ilo[1] - psc.ilg[1]) * psc.img[0]];
...
void sse2_push_part_xz()
{
  ...
  static int pr;
  if (!pr) {
    pr = prof_register("sse2_part_xz", 1., 0,
                       psc.pp.n_part * 9 * sizeof(sse2_real));
  }
  prof_start(pr);
...
///
/// Only the full push is included here. It has been converted from the yz
/// pusher, and errors may remain.
