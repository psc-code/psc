...
inline void calc_vi(struct particle_vec* p, pvReal* restrict vxi,
                    pvReal* restrict vyi, pvReal* restrict vzi,
                    pvReal* restrict root)
{
...
inline void find_index_Cround(
  pvReal* xi,         ///< Particle coordinate
  pvReal* dxi,        ///< 1/dx
...
inline void find_index_minus_shift(
  pvReal* xi,         ///< Particle coordinate
  pvReal* dxi,        ///< 1/dx
  pvInt* restrict j,  ///< Resultant coordinate on the grid
  pvReal* restrict h, ///< Difference between j - xi*dxi
  pvReal* shift)
{ ///< Amount to be subtracted from xi*dxi before finding index
  pvReal tmp;
  tmp.r = pv_mul_real(xi->r, dxi->r);
...
inline void ip_to_grid_m(pvReal* h, pvReal* restrict xmx)
{
...
/// for field interpolation on the grid
inline void ip_to_grid_O(pvReal* h, pvReal* restrict xOx)
{
...
inline void ip_to_grid_l(pvReal* h, pvReal* restrict xlx)
{
...
inline void form_factor_m(pvReal* h, pvReal* restrict xmx)
{
...
inline void form_factor_O(pvReal* h, pvReal* restrict xOx)
{
  xOx->r = pv_mul_real(h->r, h->r);
  xOx->r = pv_sub_real(threefourths.r, xOx->r);
}

inline void form_factor_l(pvReal* h, pvReal* restrict xlx)
{
...
void push_pi_dt(struct particle_vec* p, pvReal* exq, pvReal* eyq, pvReal* ezq,
                pvReal* bxq, pvReal* byq, pvReal* bzq, pvReal* dqs)
{
...
////////////////////
/// \file simd_push_common.c Certain functions common to all SIMD particle
/// pushers.
///
/// The functions contained in this file used to be declared static at the
/// the top of sse2_push_part_yz.c. As I started writing the xz pusher, I
/// relized these seem to be common among all the differenent pushers. As such,
/// there's not really any harm in giving them a seperate file. This way any bug
/// fixes don't need to be replicated seven times.
