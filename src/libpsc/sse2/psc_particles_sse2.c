...
void mparticles_sse2_free(particles_sse2_t* pp)
{
  free(pp->particles);
  pp->particles = NULL;
}

void psc_mparticles_sse2_get_sse2(particles_sse2_t* pp)
{
...
void psc_mparticles_sse2_put_sse2(particles_sse2_t* pp) {}
...
void psc_mparticles_c_get_sse2(particles_sse2_t* particles,
                               void* _particles_base)
{
  ...
  for (int n = n_part; n < (n_part + pad); n++) {
    ...
    particle_sse2_t* part = &particles->particles[n];
    part->xi = base_part->xi; // We need to be sure the padding loads fields
...
void psc_mparticles_c_put_sse2(particles_sse2_t* particles,
                               void* _particles_base)
{
  mparticles_c_t* particles_base = _particles_base;
  particles_c_t* pp_base = &particles_base->p[0];
