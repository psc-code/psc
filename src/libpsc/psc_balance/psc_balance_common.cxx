				      ...
				      int *nr_particles_by_patch_new)
{
  ...
  for (int ri = 0; ri < ctx->nr_recv_ranks; ri++) {
    ...
    for (int pi = 0; pi < recv->nr_patches; pi++) {
      ...
      typename mparticles_t::patch_t prts_new = mparticles_t(mprts_new)[p].range();
  ...
  for (int ri = 0; ri < ctx->nr_send_ranks; ri++) {
    ...
    for (int pi = 0; pi < send->nr_patches; pi++) {
      ...
      mparticles_t::patch_t prts_old = mparticles_t(mprts_old)[p].range();
  ...
  for (int p = 0; p < ctx->nr_patches_new; p++) {
    ...
    mparticles_t::patch_t prts_old = mparticles_t(mprts_old)[ctx->recv_info[p].patch].range();
    mparticles_t::patch_t prts_new = mparticles_t(mprts_new)[p].range();
