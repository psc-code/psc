...
static void
do_calc_j_1vb_yz(struct psc_fields *pf, struct psc_particles *pp)
{
  particle_real_t dt = ppsc->dt;
  particle_real_t fnqs = sqr(ppsc->coeff.alpha) * ppsc->coeff.cori / ppsc->coeff.eta;
  particle_real_t fnqys = ppsc->dx[1] * fnqs / dt;
  ...
  for (int n = 0; n < pp->n_part; n++) {
    ...
    // rewind (actually everything is at n -> n-1)
    // x^(n+1.5), p^(n+1.0) -> x^(n+0.5), p^(n+1.0) 
    calc_vxi(vxi, part);
    push_xi(part, vxi, -dt);
...
void
psc_push_particles_1vb_calc_j_yz(struct psc_push_particles *push,
				 struct psc_particles *prts_base,
				 struct psc_fields *flds_base)
{
  struct psc_particles *prts = psc_particles_get_as(prts_base, PARTICLE_TYPE, 0);
  ...
  psc_fields_zero_range(flds, JXI, JXI + 3);
  struct psc_fields *flds_cache = cache_fields_from_em(flds);
  do_calc_j_1vb_yz(flds_cache, prts);
  cache_fields_to_j(flds_cache, flds);
  psc_fields_destroy(flds_cache);
