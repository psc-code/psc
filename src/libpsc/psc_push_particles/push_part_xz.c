...
static void
do_genc_push_part_xz(int p, fields_t *pf, struct psc_particles *pp)
{
...
void
psc_push_particles_generic_c_push_a_xz(struct psc_push_particles *push,
				       ...
				       struct psc_fields *flds)
{
  ...
  do_genc_push_part_xz(prts->p, flds, prts);
  prof_stop(pr);
}
