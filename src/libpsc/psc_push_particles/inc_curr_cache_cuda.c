...
typedef fields_real_t * curr_cache_t;
...
init_curr_cache(fields_real_t *curr_cache, int ci0[3])
{
#ifdef __CUDACC__
...
__device__ static fields_real_t curr_cache_blocks[CURR_CACHE_SIZE * NR_BLOCKS];
...
CUDA_SHARED fields_real_t curr_cache_block[CURR_CACHE_SIZE];
...
curr_cache_destroy(curr_cache_t curr_cache, flds_curr_t flds_curr, int ci0[3])
{
  ...
  for (int i = threadIdx.x; i < BLOCKGSIZE_X * BLOCKGSIZE_Y * BLOCKGSIZE_Z; i += THREADS_PER_BLOCK) {
	...
	val += F3_DEV_SHIFT(curr_cache, JXI + m, ix+ci0[0],iy+ci0[1],iz+ci0[2], wid);
      ...
      fields_real_t *addr = &F(JXI + m, ix+ci0[0],iy+ci0[1],iz+ci0[2]);
      atomicAdd(addr, val);
    }
  }
#else
