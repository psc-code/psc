...
static void
do_push_part_1sff_xz(int p, fields_t *pf, particles_t *pp)
{
...
void
psc_push_particles_1sff_push_xz(struct psc_push_particles *push,
			       ...
			       mfields_base_t *flds_base)
{
  ...
  static int pr;
  if (!pr) {
    pr = prof_register("1sff_part_xz", 1., 0, 0);
  }
  prof_start(pr);
  ...
  psc_foreach_patch(ppsc, p) {
    do_push_part_1sff_xz(p, psc_mfields_get_patch(flds, p), &particles.p[p]);
  }
  prof_stop(pr);
