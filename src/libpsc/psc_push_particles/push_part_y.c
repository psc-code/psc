...
static void
do_genc_push_part_y(int p, struct psc_fields *pf, particle_range_t prts)
{
...
void
psc_push_particles_generic_c_push_mprts_y(struct psc_push_particles *push,
					  ...
					  struct psc_mfields *mflds)
{
  ...
  for (int p = 0; p < mprts->nr_patches; p++) {
    ...
    do_genc_push_part_y(p, flds, prts);
  }
  prof_stop(pr);
}
