...
static void psc_fields_acc_zero_comp(struct psc_fields* pf, int m)
{
...
static void psc_fields_acc_copy_from_single(struct psc_fields* flds_acc,
                                            struct psc_fields* flds_single,
                                            int mb, int me)
{
  for (int m = mb; m < me; m++) {
    for (int jz = flds_acc->ib[2]; jz < flds_acc->ib[2] + flds_acc->im[2];
         jz++) {
      for (int jy = flds_acc->ib[1]; jy < flds_acc->ib[1] + flds_acc->im[1];
           jy++) {
        for (int jx = flds_acc->ib[0]; jx < flds_acc->ib[0] + flds_acc->im[0];
             jx++) {
...
static void psc_fields_acc_copy_to_single(struct psc_fields* flds_acc,
                                          struct psc_fields* flds_single,
                                          int mb, int me)
{
...
static void psc_mfields_acc_setup(struct psc_mfields* mflds)
{
...
static void psc_mfields_acc_destroy(struct psc_mfields* mflds)
{
  struct psc_mfields_acc* sub = psc_mfields_acc(mflds);
