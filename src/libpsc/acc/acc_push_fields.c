...
static void acc_push_flds_E_xyz(struct psc_fields* flds)
{
    ...
    for (int iy = -1; iy < ldims[1] + 2; iy++) {
      for (int ix = -1; ix < ldims[0] + 2; ix++) {
        F3_ACC(flds, EX, ix, iy, iz) +=
          +cny *
            (F3_ACC(flds, HZ, ix, iy, iz) - F3_ACC(flds, HZ, ix, iy - 1, iz)) -
          cnz *
            (F3_ACC(flds, HY, ix, iy, iz) - F3_ACC(flds, HY, ix, iy, iz - 1)) -
          .5 * ppsc->dt * F3_ACC(flds, JXI, ix, iy, iz);

        F3_ACC(flds, EY, ix, iy, iz) +=
          +cnz *
            (F3_ACC(flds, HX, ix, iy, iz) - F3_ACC(flds, HX, ix, iy, iz - 1)) -
          cnx *
            (F3_ACC(flds, HZ, ix, iy, iz) - F3_ACC(flds, HZ, ix - 1, iy, iz)) -
          .5 * ppsc->dt * F3_ACC(flds, JYI, ix, iy, iz);

        F3_ACC(flds, EZ, ix, iy, iz) +=
          +cnx *
            (F3_ACC(flds, HY, ix, iy, iz) - F3_ACC(flds, HY, ix - 1, iy, iz)) -
          cny *
            (F3_ACC(flds, HX, ix, iy, iz) - F3_ACC(flds, HX, ix, iy - 1, iz)) -
...
static void acc_push_flds_E_yz(struct psc_fields* flds)
{
  struct psc_patch* patch = &ppsc->patch[flds->p];
  fields_acc_real_t cny = .5 * ppsc->dt / patch->dx[1];
  fields_acc_real_t cnz = .5 * ppsc->dt / patch->dx[2];
...
static void acc_push_flds_H_xyz(struct psc_fields* flds)
{
  struct psc_patch* patch = &ppsc->patch[flds->p];
  fields_acc_real_t cnx = .5 * ppsc->dt / patch->dx[0];
  fields_acc_real_t cny = .5 * ppsc->dt / patch->dx[1];
    ...
    for (int iy = -1; iy < ldims[1] + 1; iy++) {
      for (int ix = -1; ix < ldims[0] + 1; ix++) {
        F3_ACC(flds, HX, ix, iy, iz) -=
          +cny *
            (F3_ACC(flds, EZ, ix, iy + 1, iz) - F3_ACC(flds, EZ, ix, iy, iz)) -
          cnz *
            (F3_ACC(flds, EY, ix, iy, iz + 1) - F3_ACC(flds, EY, ix, iy, iz));

        F3_ACC(flds, HY, ix, iy, iz) -=
          +cnz *
            (F3_ACC(flds, EX, ix, iy, iz + 1) - F3_ACC(flds, EX, ix, iy, iz)) -
          cnx *
            (F3_ACC(flds, EZ, ix + 1, iy, iz) - F3_ACC(flds, EZ, ix, iy, iz));

        F3_ACC(flds, HZ, ix, iy, iz) -=
          +cnx *
            (F3_ACC(flds, EY, ix + 1, iy, iz) - F3_ACC(flds, EY, ix, iy, iz)) -
          cny *
            (F3_ACC(flds, EX, ix, iy + 1, iz) - F3_ACC(flds, EX, ix, iy, iz));
      }
    }
  }
}
...
static void acc_push_flds_H_yz(struct psc_fields* flds)
{
  struct psc_patch* patch = &ppsc->patch[flds->p];
  fields_acc_real_t cny = .5 * ppsc->dt / patch->dx[1];
  fields_acc_real_t cnz = .5 * ppsc->dt / patch->dx[2];
