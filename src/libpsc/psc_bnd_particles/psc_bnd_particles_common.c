...
psc_bnd_particles_open_boundary(struct psc_bnd_particles *bnd, struct psc_mparticles *mprts)
{
  ...
  psc_mfields_set_param_int(mflds_ipr, "nr_fields", 6 * nr_kinds); // FIXME
  ...
  for (int p = 0; p < ppsc->nr_patches; p++) {
	...
	for (int iy = 0; iy < ppatch->ldims[1]; iy++) {
	  ...
	  double  vsz=sqrt(2.0*(F3_C(flds_nvt_av, 10*m + 6, 0,iy,0)));
	  double  gs0=exp(-vzea*vzea/vsz/vsz)-exp(-(c-vzea)*(c-vzea)/vsz/vsz)
	    +sqrt(M_PI)*vzea/vsz*(erf((c-vzea)/vsz)+erf(vzea/vsz)) ;
	  ninjo=F3_C(flds_n_in, m, 0,iy,0);
	  //F3_C(flds_nvt_av, 10*m + 0, 0,iy,0) = 1.;//!!!
	  F3_C(flds_n_in, m, 0,iy,0) += ppsc->dt*gs0*F3_C(flds_nvt_av, 10*m + 0, 0,iy,0)
	    *vsz/sqrt(M_PI)/2.0/ppsc->patch[p].dx[2]/ppsc->coeff.cori;
	  ninjn=F3_C(flds_n_in, m, 0,iy,0);
	  int ninjc=0;
	  if(ninjo < ninjn){
	    ninjc=ninjn-ninjo;
	  }
	  ...
	  if(ninjc!=0) {
		...
		prt->pxi=vxea+yya*sqrt(F3_C(flds_ipr, 6*m + 1, 0,iy,0)/(F3_C(flds_ipr, 6*m + 0, 0,iy,0)*F3_C(flds_ipr, 6*m + 1, 0,iy,0)
									-sqr(F3_C(flds_ipr, 6*m + 3, 0,iy,0))))
		...
		prt->pyi=vyea+1.0/F3_C(flds_ipr, 6*m + 1, 0,iy,0)*(sqrt(F3_C(flds_ipr, 6*m + 1, 0,iy,0))*yya
								   -(prt->pzi-vzea)*F3_C(flds_ipr, 6*m + 5, 0,iy,0)-(prt->pxi-vxea)*F3_C(flds_ipr, 6*m + 3, 0,iy,0));
		...
		if(nnm>100) break;
	      ...
	      seed=random();
	      sr=((double) seed)/((double)RAND_MAX);
	      prt->zi=sr*prt->pzi * ppsc->dt;//ppsc->patch[p].dx[2];
