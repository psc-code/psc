...
struct MparticlesCuda : MparticlesBase
{
  ...
  void inject_buf(const particle_t *buf, const std::vector<uint>& buf_n_by_patch);
  ...
  struct patch_t
  {
	...
	if (patch_.p_ == mprts.n_patches() - 1) {
	  mprts.inject_buf(mprts.injector_buf.data(), mprts.injector_n_prts_by_patch);
