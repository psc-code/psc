...
struct InjectorBuffered
{
  ...
  struct Patch
  {
    Patch(InjectorBuffered& injector, int p)
      : injector_(injector), p_{p}, n_prts_{0} // FIXME, why (), {} does not
                                               // work
    {}

    ~Patch() { injector_.n_prts_by_patch_[p_] += n_prts_; }
    ...
    void operator()(const psc::particle::Inject& new_prt)
    {
      ...
      injector_.buf_.push_back({Real3(x), Real3(u), q * real_t(new_prt.w),
                                new_prt.kind, mprts.uid_gen(), new_prt.tag});
      n_prts_++;
    }

    // FIXME do we want to keep this? or just have a psc::particle::Inject
    // version instead?
  ...
  void reserve(int n_prts_total) { buf_.reserve(n_prts_total); }
