...
void _cuda_mfields_const_set(struct cuda_mfields* cmflds)
{
  ...
  cudaError_t ierr = cudaMemcpyToSymbol(_d_cmflds_const, &c, sizeof(c));
  cudaCheck(ierr);
}
...
struct _DFields
{
  ...
  __host__ __device__ _DFields(real_t* d_flds) : d_flds_(d_flds) {}

  __device__ real_t operator()(int m, int i, int j, int k) const
  {
    return d_flds_[index(m, i, j, k)];
  }
  __device__ real_t& operator()(int m, int i, int j, int k)
  {
    return d_flds_[index(m, i, j, k)];
  }
...
private:
  ...
  {
    return ((((m)*_d_cmflds_const.im[2] + (k + 2)) * _d_cmflds_const.im[1] +
             (j + 2)) *
              1 +
            (0));
  }
...
struct _DMFields
{
  ...
  __host__ __device__ _DFields operator[](int p)
  {
    return _DFields(d_flds_ + p * stride_);
  }
