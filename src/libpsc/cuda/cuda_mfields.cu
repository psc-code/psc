...
void cuda_mfields::copy_comp_yz(int ym, cuda_mfields* cmflds_x, int xm)
{
  ...
  for (int p = 0; p < n_patches(); p++) {
    k_copy_comp_yz<<<dimGrid, dimBlock>>>((*this)[p].storage().data().get(), ym,
                                          (*cmflds_x)[p].storage().data().get(),
                                          xm, my, mz);
  }
...
void cuda_mfields::copy_comp_xyz(int ym, cuda_mfields* cmflds_x, int xm)
{
  ...
  for (int p = 0; p < n_patches(); p++) {
    k_copy_comp_xyz<<<dimGrid, dimBlock>>>(
      (*this)[p].storage().data().get(), ym,
      (*cmflds_x)[p].storage().data().get(), xm, mx, my, mz);
  }
  cuda_sync_if_enabled();
}
...
void cuda_mfields::axpy_comp_yz(int ym, float a, cuda_mfields* cmflds_x, int xm)
{
  ...
  for (int p = 0; p < n_patches(); p++) {
    k_axpy_comp_yz<<<dimGrid, dimBlock>>>(
      (*this)[p].storage().data().get(), ym, a,
      (*cmflds_x)[p].storage().data().get(), xm, my, mz);
  }
  cuda_sync_if_enabled();
}
