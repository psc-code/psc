...
__global__ static void
mprts_find_block_indices_2_total(struct cuda_mparticles_params mprts_prm, float4 *d_xi4,
				 unsigned int *d_off,
				 unsigned int *d_bidx, int nr_patches)
{
  ...
  int bid = block_pos_to_block_idx(block_pos, mprts_prm.b_mx);
  int p = blockIdx.y / mprts_prm.b_mx[2];

  int nr_blocks = mprts_prm.b_mx[1] * mprts_prm.b_mx[2];
  ...
  for (int n = block_begin + tid; n < block_end; n += THREADS_PER_BLOCK) {
    float4 xi4 = d_xi4[n];
    unsigned int block_pos_y = __float2int_rd(xi4.y * mprts_prm.b_dxi[1]);
    unsigned int block_pos_z = __float2int_rd(xi4.z * mprts_prm.b_dxi[2]);

    int block_idx;
    if (block_pos_y >= mprts_prm.b_mx[1] || block_pos_z >= mprts_prm.b_mx[2]) {
      block_idx = nr_blocks * nr_patches;
    } else {
      block_idx = block_pos_z * mprts_prm.b_mx[1] + block_pos_y + p * nr_blocks;
    }
    d_bidx[n] = block_idx;
  }
}
...
__global__ static void
mprts_find_block_keys(struct cuda_mparticles_params mprts_prm, float4 *d_xi4,
		      unsigned int *d_off,
		      unsigned int *d_bidx, int nr_total_blocks)
{
  ...
  for (int n = block_begin + tid; n < block_end; n += THREADS_PER_BLOCK) {
    ...
    if (block_pos_y >= mprts_prm.b_mx[1] || block_pos_z >= mprts_prm.b_mx[2]) {
      ...
      int bidx = block_pos_z * mprts_prm.b_mx[1] + block_pos_y + p * nr_blocks;
      int b_diff = bid - bidx + mprts_prm.b_mx[1] + 1;
      int d1 = b_diff % mprts_prm.b_mx[1];
      int d2 = b_diff / mprts_prm.b_mx[1];
      block_idx = d2 * 3 + d1;
    }
    d_bidx[n] = block_idx;
  }
