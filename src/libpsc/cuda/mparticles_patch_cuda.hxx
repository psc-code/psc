...
struct ConstParticleAccessorCuda_
{
  ...
  real_t w() const
  {
    return prt_.qni_wni() / prts_.grid().kinds[prt_.kind()].q;
  }
...
struct ConstAccessorCuda_
{
  ...
  struct Patch
  {
    ...
    {
      const_iterator(const MparticlesPatch& prts, uint n) : prts_{prts}, n_{n}
      {}
      ...
      {
        return {const_cast<Mparticles&>(prts_.mprts).get_particle(prts_.p, n_),
                prts_};
      } // FIXME constness
  ...
  ConstAccessorCuda_(Mparticles& mprts) : mprts_{mprts} {}
