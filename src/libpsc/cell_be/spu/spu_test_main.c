...
static int spu_run_job(unsigned long long ea)
{
  ...
  switch (psc_block.job) {
    case SPU_HELLO: return spu_psc_hello(); break;
    case SPU_BYE: return spu_psc_goodbye(); break;
    default:
#ifndef NDEBUG
             ...
             unsigned long long env)
{
  ...
  //    env); fprintf(stderr, "spu main [%#llx] spu_ctx %p size %d \n", spe_id,
  //    &spu_ctx, sizeof(spu_ctx));
