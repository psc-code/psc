...
#define F2_SPU_OFF(fldnr, jx, jy)                                              \
  (((((fldnr)*psc_block.im[1] + ((jy)-psc_block.ib[1])) * psc_block.im[0] +    \
     ((jx)-psc_block.ib[0]))))
  ...
  do {
    ...
    */
    ...
    for (int p = 0; p < VEC_SIZE; p++) {
      s1x[(int)dfx[p] + 1] =
        spu_sel(s1x[(int)dfx[p] + 1], gmx,
                (vector unsigned long long)element_assign[p]);
      s1x[(int)dfx[p] + 2] =
        spu_sel(s1x[(int)dfx[p] + 2], gOx,
                (vector unsigned long long)element_assign[p]);
      s1x[(int)dfx[p] + 3] =
        spu_sel(s1x[(int)dfx[p] + 3], glx,
                (vector unsigned long long)element_assign[p]);
      s1y[(int)dfy[p] + 1] =
    ...
    for (int l2i = l2min; l2i <= l2max; l2i++) {
      ...
      long int store_off_0 =
        F2_SPU_OFF(ls_JXI, j1_scal[0] - 2, j2_scal[0] + l2i - 2);
      long int store_off_1 =
        F2_SPU_OFF(ls_JXI, j1_scal[1] - 2, j2_scal[1] + l2i - 2);
...
#define CALC_J_Z                                                               \
  {                                                                            \
    ...
    wz = spu_mul(fnqz, wz);                                                    \
      ...
      sjy0_a =
        spu_sel(zero, jyh[l1i], (vector unsigned long long)element_assign[0]);
      ...
      sjx1_a =
        spu_sel(sjx1_a, jxh, (vector unsigned long long)element_assign[1]);
      sjy1_a =
        spu_sel(sjy1_a, jyh[l1i], (vector unsigned long long)element_assign[1]);
      sjz1_a =
        spu_sel(sjz1_a, wz, (vector unsigned long long)element_assign[1]);
      ...
      sjx1_b =
        spu_sel(sjx1_b, jxh, (vector unsigned long long)element_assign[1]);
      sjy1_b =
        spu_sel(sjy1_b, jyh[l1i], (vector unsigned long long)element_assign[1]);
      sjz1_b =
        spu_sel(sjz1_b, wz, (vector unsigned long long)element_assign[1]);
