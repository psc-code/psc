...
begin_initialization
{
  ...
  double Te =
    me * c * c /
    (2 * eps0 * wpe_wce * wpe_wce * (1 + Ti_Te)); // Electron temperature
  ...
  double vthib =
    sqrt(Tbi_Ti * Ti / mi); // normalized background ion thermal vel.
  ...
  double gdre =
    1 / sqrt(1 - vdre * vdre / (c * c)); // gamma of electron drift frame
  ...
  double dbz =
    dbz_b0 * b0; // Perturbation in Bz relative to Bo (Only change here)
  double dbx =
    -dbz * Lpert / (2.0 * Lz); // Set Bx perturbation so that div(B) = 0
  ...
  double cvar =
    dt_coll * 3.0 * sqrt(2.0 * M_PI) / 4.0 * pow(vthe, 3) * nuei_wce / wpe_wce;
...
#define RANK_TO_INDEX(rank, ix, iy, iz)                                        \
  ...
  // if ( iz==0 )            set_domain_field_bc( BOUNDARY(0,0,-1),
  // absorb_fields ); if ( iz==topology_z-1 ) set_domain_field_bc( BOUNDARY(
  // 0,0,1), absorb_fields );
  if (global->open_bc_x) {
  ...
  // ); if ( ix==topology_x-1 ) set_domain_field_bc( BOUNDARY( 1,0,0),
  // pec_fields
  // );
...
#define resistive_layer                                                        \
  ((global->open_bc_x && x < hx * thickness) ||                                \
  ...
  if (rank() == 0) {
    FileIO fp_info;
    if (!(fp_info.open("info", io_write) == ok))
  ...
  set_region_field(
    everywhere, 0, 0, 0, // Electric field
    cs * b0 * tanh(z / L) +
      dbx * cos(2.0 * M_PI * (x - 0.5 * Lx) / Lpert) * sin(M_PI * z / Lz), // Bx
  ...
  //      set_region_field( everywhere, 0, 0, 0,                    // Electric
  //      field
  ...
  repeat(Ne_back / nproc())
  {
    ...
    inject_particle(electron, x, y, z, normal(rng(0), 0, vtheb),
                    normal(rng(0), 0, vtheb), normal(rng(0), 0, vtheb),
                    weight_b, 0, 0);

    inject_particle(ion, x, y, z, normal(rng(0), 0, vthib),
                    normal(rng(0), 0, vthib), normal(rng(0), 0, vthib),
                    weight_b, 0, 0);
  }
