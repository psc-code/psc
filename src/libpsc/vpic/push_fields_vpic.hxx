...
struct PushFieldsVpicWrap
{
  void push_E(MfieldsState& mflds, double dt_fac)
  {
    ...
    TIC return fa->kernel->advance_e(fa, dt_fac);
    TOC(advance_e, 1);
#if 0
  ...
  {
    field_array_t* fa = mflds;
    TIC return fa->kernel->advance_b(fa, dt_fac);
    TOC(advance_b, 1);
  }
};
...
#undef EZ
  ...
  static void vacuum_advance_e(MfieldsState& mflds, double frac)
  {
    ...
    struct AdvanceE
    {
      AdvanceE(typename MfieldsState::Patch& fa, const Grid* g,
               const MaterialCoefficient* m, const double damp_)
        : F(fa),
          decayx(m->decayx),
          decayy(m->decayy),
          decayz(m->decayz),
          ...
          drivey(m->drivey),
          drivez(m->drivez),
          px_muz(g->nx > 1 ? (1 + damp_) * g->cvac * g->dt * g->rdx * m->rmuz
                           : 0),
          px_muy(g->nx > 1 ? (1 + damp_) * g->cvac * g->dt * g->rdx * m->rmuy
                           : 0),
          py_mux(g->ny > 1 ? (1 + damp_) * g->cvac * g->dt * g->rdy * m->rmux
                           : 0),
          py_muz(g->ny > 1 ? (1 + damp_) * g->cvac * g->dt * g->rdy * m->rmuz
                           : 0),
          pz_muy(g->nz > 1 ? (1 + damp_) * g->cvac * g->dt * g->rdz * m->rmuy
                           : 0),
          pz_mux(g->nz > 1 ? (1 + damp_) * g->cvac * g->dt * g->rdz * m->rmux
                           : 0),
          damp(damp_),
          cj(g->dt / g->eps0)
      {}
      ...
      void x(int i, int j, int k)
      {
        F(i, j, k).tcax = ((py_muz * (F(i, j, k).cbz - F(i, j - 1, k).cbz) -
                           ...
                           damp * F(i, j, k).tcax);
        F(i, j, k).ex = decayx * F(i, j, k).ex +
                        drivex * (F(i, j, k).tcax - cj * F(i, j, k).jfx);
      }
      ...
      void y(int i, int j, int k)
      {
        F(i, j, k).tcay = ((pz_mux * (F(i, j, k).cbx - F(i, j, k - 1).cbx) -
                           ...
                           damp * F(i, j, k).tcay);
        F(i, j, k).ey = decayy * F(i, j, k).ey +
                        drivey * (F(i, j, k).tcay - cj * F(i, j, k).jfy);
      }
      ...
      void z(int i, int j, int k)
      {
        F(i, j, k).tcaz = ((px_muy * (F(i, j, k).cby - F(i - 1, j, k).cby) -
                           ...
                           damp * F(i, j, k).tcaz);
        F(i, j, k).ez = decayz * F(i, j, k).ez +
                        drivez * (F(i, j, k).tcaz - cj * F(i, j, k).jfz);
      }
