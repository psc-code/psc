...
struct PscSfaParams
{
  ...
  PscSfaParams(const Grid* g, const MaterialList& m_list, float damp)
  {
    ...
    float ax = g->nx > 1 ? g->cvac * g->dt * g->rdx : 0;
    ax *= ax;
    float ay = g->ny > 1 ? g->cvac * g->dt * g->rdy : 0;
    ay *= ay;
    float az = g->nz > 1 ? g->cvac * g->dt * g->rdz : 0;
    az *= az;
    int n_mc = 0;
    ...
    for (auto m = m_list.cbegin(); m != m_list.cend(); ++m) {
      ...
      if (ax == 0)
        ...
        mc->drivex = 2. * exp(-0.5 * ax) * sinh(0.5 * ax) / (ax * m->epsx);
      if (ay == 0)
        mc->drivey = 1. / m->epsy;
      else if (mc->decayy == 0)
        mc->drivey = 0;
      else
        mc->drivey = 2. * exp(-0.5 * ay) * sinh(0.5 * ay) / (ay * m->epsy);
      if (az == 0)
        mc->drivez = 1. / m->epsz;
      else if (mc->decayz == 0)
        mc->drivez = 0;
      else
        mc->drivez = 2. * exp(-0.5 * az) * sinh(0.5 * az) / (az * m->epsz);
      mc->rmux = 1. / m->mux;
      mc->rmuy = 1. / m->muy;
      mc->rmuz = 1. / m->muz;
...
struct MfieldsStatePsc
{
  ...
  MfieldsStatePsc(const Grid_t& grid, Grid* vgrid,
                  const MaterialList& material_list, double damp = 0.)
    : grid_{grid}, patch_{vgrid}, params_{vgrid, material_list, real_t(damp)}
  {
  ...
  {
    return *patch_.grid();
  } // FIXME, const version would be better
