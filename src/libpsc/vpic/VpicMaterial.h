...
struct VpicMaterialList : public VpicListBase<VpicMaterial>
{
  ...
  static Material* create(const char* name, float epsx, float epsy, float epsz,
                          ...
                          float zetaz)
  {
    return static_cast<Material*>(::material(name, epsx, epsy, epsz, mux, muy,
                                             muz, sigmax, sigmay, sigmaz, zetax,
                                             zetay, zetaz));
  }
  ...
  Material* append(Material* m)
  {
    return static_cast<Material*>(
      ::append_material(m, reinterpret_cast<material_t**>(&head_)));
  }

  operator const material_t*() const { return head_; }
};
