...
struct PscAccumulatorOps
{
  ...
  static void unload(/*const*/ MfieldsAccumulator& acc, MfieldsState& mflds)
  {
    ...
    // FIXME, these limits seem to go too far out compared to what we zeroed
    // before
    for (int k = 1; k <= nz + 1; k++) {
      for (int j = 1; j <= ny + 1; j++) {
        for (int i = 1; i <= nx + 1; i++) {
          F(i, j, k).jfx +=
            cx * (A(i, j, k).jx[0] + A(i, j - 1, k).jx[1] +
                  A(i, j, k - 1).jx[2] + A(i, j - 1, k - 1).jx[3]);
          F(i, j, k).jfy +=
            cy * (A(i, j, k).jy[0] + A(i, j, k - 1).jy[1] +
                  A(i - 1, j, k).jy[2] + A(i - 1, j, k - 1).jy[3]);
          F(i, j, k).jfz +=
            cz * (A(i, j, k).jz[0] + A(i - 1, j, k).jz[1] +
                  A(i, j - 1, k).jz[2] + A(i - 1, j - 1, k).jz[3]);
        }
      }
    }
  }
