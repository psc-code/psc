...
// ======================================================================
// psc_inject subclass "sub"

// ----------------------------------------------------------------------
// psc_inject_sub_create

static void
psc_inject_sub_create(struct psc_inject *inject)
{
...
static void
psc_inject_sub_run(struct psc_inject *inject, struct psc_mparticles *mprts_base,
  ...
  psc_foreach_patch(psc, p) {
	...
	for (int jx = 0; jx < ldims[0]; jx++) {
	  ...
	  for (int kind = 0; kind < nr_pop; kind++) {
	    ...
	    mprts[p].reserve(mprts[p].size() + n_in_cell);
...
struct psc_inject_ops_sub : psc_inject_ops {
  psc_inject_ops_sub() {
    name                = PARTICLE_TYPE;
    create              = psc_inject_sub_create;
    run                 = psc_inject_sub_run;
  }
} psc_inject_ops_sub;
