			     ...
			     int outtype, float sheet)
{
  ...
  if (fld->_len) { // part of the slice?
    ...
    MPI_Send(iw, 6, MPI_INT, par->rank_diagsrv, ID_DIAGS_SUBDOMAIN, MPI_COMM_WORLD);
    struct mrc_fld *f = mrc_fld_get_as(fld, "float");
    MPI_Send(fld->_nd->arr, fld->_len, MPI_FLOAT, par->rank_diagsrv, ID_DIAGS_2DDATA, MPI_COMM_WORLD);
    mrc_fld_put_as(f, fld);
  } else {
    MPI_Send(iw, 6, MPI_INT, par->rank_diagsrv, ID_DIAGS_SUBDOMAIN, MPI_COMM_WORLD);
  }
  ...
  case PT_DOUBLE:
	...
	for (int k = 0; k < nr_procs; k++) {
	  ...
	  if (iw[0] > -1) {
	    ...
	    MPI_Recv(_lfld2->_nd->arr, lfld2->_len, MPI_FLOAT, k, ID_DIAGS_2DDATA, MPI_COMM_WORLD,
		     MPI_STATUS_IGNORE);
	...
	for (int k = 0; k < nr_procs; k++) {
	  ...
	  if (iw[0] > -1) {
	    ...
	    MPI_Recv(lfld3->_nd->arr, lfld3->_len, MPI_FLOAT, k, ID_DIAGS_DATA, MPI_COMM_WORLD,
		     MPI_STATUS_IGNORE);
