...
static void psc_bubble_create(struct psc* psc)
{
...
static void psc_bubble_set_from_options(struct psc* psc)
{
...
static double psc_bubble_init_field(struct psc* psc, double x[3], int m)
{
  struct psc_bubble* bubble = to_psc_bubble(psc);
...
static void psc_bubble_init_npt(struct psc* psc, int kind, double x[3],
                                struct psc_particle_npt* npt)
{
  ...
  switch (kind) {
    case 0: // electrons
      ...
      if ((r1 <= LLn) && (r1 >= LLn - 2. * LLB)) {
        npt->p[1] = -BB * M_PI / (2. * LLB) *
                    cos(M_PI * (LLn - r1) / (2. * LLB)) / npt->n;
      }
      if ((r2 <= LLn) && (r2 >= LLn - 2. * LLB)) {
        npt->p[1] = -BB * M_PI / (2. * LLB) *
                    cos(M_PI * (LLn - r2) / (2. * LLB)) / npt->n;
      }
