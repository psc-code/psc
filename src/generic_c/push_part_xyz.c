...
static void
do_genc_push_part_xyz(fields_t *pf, particles_t *pp)
{
...
void
genc_push_part_xyz(mfields_base_t *flds_base, mparticles_base_t *particles_base)
{
  mfields_t flds;
  ...
  prof_start(pr);
  foreach_patch(p) {
    do_genc_push_part_xyz(&flds.f[p], &particles.p[p]);
  }
  prof_stop(pr);
