...
static struct psc_mfields* make_mfields(MPI_Comm comm)
{
...
static void test_create_destroy(MPI_Comm comm)
{
  struct psc_mfields* mflds = psc_mfields_create(comm);
...
static void test_setup(MPI_Comm comm)
{
...
static void test_print(MPI_Comm comm)
{
  struct psc_mfields* mflds = make_mfields(comm);
  psc_mfields_view(mflds);
    ...
    for (int m = 0; m < flds.nr_comp; m++) {
      for (int jz = flds.ib[2]; jz < flds.ib[2] + flds.im[2]; jz++) {
        for (int jy = flds.ib[1]; jy < flds.ib[1] + flds.im[1]; jy++) {
          mprintf("mflds[%d, %d:%d,%d,%d, %d] =", m, flds.ib[0],
                  flds.ib[0] + flds.im[0], jy, jz, p);
