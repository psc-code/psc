...
static void psc_test_em_wave_create(struct psc* psc)
{
...
static void psc_test_em_wave_setup(struct psc* psc)
{
...
static double psc_test_em_wave_init_field(struct psc* psc, double crd[3], int m)
{
  struct psc_test_em_wave* sub = psc_test_em_wave(psc);
  double *k = sub->k, *B = sub->B, *E = sub->E;
...
static int is_equal_tol(fields_t::real_t tol, fields_t::real_t v1,
                        fields_t::real_t v2)
{
  if (fabs(v1 - v2) < tol) {
    return 0;
...
static void psc_test_em_wave_check(struct psc* psc)
{
  struct psc_test_em_wave* sub = psc_test_em_wave(psc);
  ...
  for (int p = 0; p < mf.n_patches(); p++) {
    ...
    {
      double dx = psc->patch[p].dx[0], dy = psc->patch[p].dx[1],
             dz = psc->patch[p].dx[2];
      double xx = CRDX(p, jx), yy = CRDY(p, jy), zz = CRDZ(p, jz);

      failed += is_equal_tol(tol, F(EX, jx, jy, jz),
                             E[0] * sin(om * t - (k[0] * (xx + .5f * dx) +
                                                  k[1] * (yy) + k[2] * (zz))));
      failed += is_equal_tol(
        tol, F(EY, jx, jy, jz),
        E[1] *
          sin(om * t - (k[0] * (xx) + k[1] * (yy + .5f * dy) + k[2] * (zz))));
      failed += is_equal_tol(tol, F(EZ, jx, jy, jz),
                             E[2] * sin(om * t - (k[0] * (xx) + k[1] * (yy) +
                                                  k[2] * (zz + .5f * dz))));

      failed +=
        is_equal_tol(tol, F(HX, jx, jy, jz),
                     B[0] * sin(om * t - (k[0] * (xx) + k[1] * (yy + .5f * dy) +
                                          k[2] * (zz + .5f * dz))));
      failed +=
        is_equal_tol(tol, F(HY, jx, jy, jz),
                     B[1] * sin(om * t - (k[0] * (xx + .5f * dx) + k[1] * (yy) +
                                          k[2] * (zz + .5f * dz))));
      failed += is_equal_tol(
        tol, F(HZ, jx, jy, jz),
        B[2] * sin(om * t - (k[0] * (xx + .5f * dx) + k[1] * (yy + .5f * dy) +
                             k[2] * (zz))));
    }
    foreach_3d_end;
  }
...
static void psc_test_em_wave_check_single(struct psc* psc)
{
  struct psc_test_em_wave* sub = psc_test_em_wave(psc);
  double om = -sub->om; // FIXME, looks like we need a minus sign here, but why?
  double t = psc->timestep * psc->dt;
...
static void psc_test_em_wave_check_vpic(struct psc* psc)
{
#if 0
  struct psc_test_em_wave *sub = psc_test_em_wave(psc);
  double om = -sub->om; // FIXME, looks like we need a minus sign here, but why?
...
static void psc_test_em_wave_step(struct psc* psc)
{
  psc_output(psc);
