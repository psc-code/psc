...
void
psc_testing_push_yz_b(struct psc *psc, const char *s_push_particles)
{
  if (opt_checks_verbose) {
    mprintf("=== testing push_part_yz_b() %s\n", s_push_particles);
  }
...
// ----------------------------------------------------------------------
// check push_particles_push_yz_b against "fortran" ref
...
main(int argc, char **argv)
{
  ...
  psc_setup(psc);
  psc_testing_push_yz_b(psc, "fortran");
  psc_testing_save_ref(psc);
  psc_destroy(psc);

  psc = psc_testing_create_test_yz(s_type, 0, "c");
  psc_setup(psc);
  psc_testing_push_yz_b(psc, s_type);
