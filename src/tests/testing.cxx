...
static struct psc_mfields* fld_create(struct psc* psc, int nr_fields)
{
...
void __assert_equal(double x, double y, const char* xs, const char* ys,
                    double thres)
{
  ...
  if (eps > thres) {
    fprintf(stderr,
            "assert_equal: fail %s = %g, %s = %g rel err = %g thres = %g\n", xs,
            x, ys, y, eps, thres);
    abort();
  }
}
...
void psc_save_particles_ref(struct psc* psc, struct psc_mparticles* mprts_base)
{
  if (!mprts_ref) {
...
void psc_save_fields_ref(struct psc* psc, struct psc_mfields* mflds_base)
{
  int me = HZ + 1;
...
void psc_check_particles_ref(struct psc* psc, struct psc_mparticles* mprts_base,
...
void psc_check_fields_ref(struct psc* psc, struct psc_mfields* mflds_base,
                          int* m_flds, double thres)
{
...
void psc_check_currents_ref(struct psc* psc, struct psc_mfields* mflds_base,
                            double thres, int sw)
{
#if 0
...
void psc_testing_check_densities_ref(struct psc* psc,
                                     ...
                                     double eps)
{
  ...
  struct psc_output_fields_item* item =
    psc_output_fields_item_create(psc_comm(psc));
...
void psc_check_particles_sorted(struct psc* psc,
                                struct psc_mparticles* mprts_base)
{
  ...
  psc_foreach_patch(psc, p)
  {
    ...
    PARTICLE_ITER_LOOP(prt_iter, prts.begin(), prts.end())
    {
      ...
      int cni =
        ((j2) * (ldims[1] + 2 * ibn[1]) + j1) * (ldims[0] + 2 * ibn[0]) + j0;
...
struct psc* psc_testing_create_test_yz(const char* s_push_particles,
                                       unsigned int mask)
{
...
void psc_testing_push_particles_check(struct psc* psc, double eps_particles,
                                      double eps_fields)
{
