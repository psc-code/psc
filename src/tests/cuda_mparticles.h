...
#define cuda_sync_if_enabled()                                                 \
  do {                                                                         \
    if (1) {                                                                   \
      cudaError_t ierr = cudaThreadSynchronize();                              \
      cudaCheck(ierr);                                                         \
    }                                                                          \
  } while (0)
...
struct cuda_mparticles
{
  ...
  // temporary: particles in each patch, used before initial sort, ie.,
  // particles are arranged by patch order only, but not sorted by block yet, so
  // d_off isn't set yet, either
...
void cuda_mparticles_find_block_indices_ids_total(
  struct cuda_mparticles* cuda_mprts, unsigned int* nr_parts_by_patch);
