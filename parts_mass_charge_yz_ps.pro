...
for ql=0L,qnum do begin
   ...
   if (pnum gt 0) then begin
      ...
      fo=strmid(nmfc,0,strlen(nmfc)-13)+'_yz'$
      ...
      plot, ypl, zpl, xs=1, ys=1, charsize=1.6, psym=1, $
...
;              strtrim(string(1.e6*z(z2)),2)], $
      ...
      line=fltarr(y2-y1+2)
      fline=fltarr(y2-y1+2)
      gline=fltarr(y2-y1+2)
      for i=0L,y2-y1+1 do begin
         line(i)=1.0e6*i*dy*ld
         fline(i)=10.27
         gline(i)=12.81
      endfor
;      oplot, line, fline, color=1, thick=2.0
;      oplot, line, gline, color=1, thick=2.0
